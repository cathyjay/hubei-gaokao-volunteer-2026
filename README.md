...
- Round4 家庭阅读说明表：`data/exports/issue19-round4-family-explanation-board.xlsx`、`data/exports/issue19-round4-family-explanation-board.csv`、`data/exports/issue19-round4-family-explanation-focus55.csv` 和 `data/exports/issue19-round4-family-explanation-paused65.csv`。把 120 组解释成重点核验 55 和暂缓保留 65，补充“为什么入选/为什么暂缓”、完整组内专业接受/勉强调剂/待核/不能计数和调剂风险说明；仍不作为定稿依据。
...
- 第一闭环核验结果看板：`data/working/issue19-first-closure-verification-result-public-ledger.csv`、`data/working/issue19-first-closure-verification-result-page-summary.csv` 和 `data/working/issue19-first-closure-verification-result-summary.json`。把 206 条最高风险任务和 37 个页列统一展示为 PDF 原页、OCR、机器坐标、高校辅证、湖北官方侧、冲突状态和写回门禁；只用于核验排程，不确认字段事实。
- 第一闭环字段级公开状态：`data/working/issue19-first-closure-field-verification-status-public-ledger.csv` 和 `data/working/issue19-first-closure-field-verification-status-summary.json`。把 206 条组合任务拆成 354 个字段级状态：专业计划数 170、学费 105、再选科目 77、待人工判定字段 2；352 个字段已映射字段事实核验任务，2 个保持人工判定口径，全部仍待 PDF 原页和湖北官方侧闭环。
- 第一闭环人工核验执行工作簿：`data/exports/issue19-first-closure-manual-verification-workbook.xlsx`，配套 `page-packets/task-items/field-items` 三张 CSV。把 37 个页列、206 个任务、354 个字段拆成可人工执行的小包，公开层只保留状态、计数、ID 和 SHA，不保存具体核验内容。
- Priority55 家庭逐专业决策工作簿：`data/exports/issue19-priority55-family-major-decision-workbook.xlsx`。把 55 个重点组、458 条完整组内专业转成家庭可标注表，供填写“可接受/勉强接受/不能接受/待了解”和服从调剂态度；仍不是最终志愿方案。
- 高校官网结构化接入候选账本：`data/working/issue19-school-source-structured-ingestion-candidates-public-ledger.csv` 和 `data/exports/issue19-school-source-structured-ingestion-candidates.xlsx`。优先列出 12 所已有 JSON/PDF/XLSX/HTML 公开源的学校，安排下一步 parser、adapter 和 candidate diff；高校源仍只作 double check，不替代湖北官方计划。
- 高校源 Adapter/Diff 执行工作台：`data/working/issue19-school-source-adapter-diff-execution-workbench-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-diff-execution-workbench-v1-summary.json`。把 12 所结构化接入候选进一步拆成 adapter、parser、normalized bridge 和候选 diff 执行动作，回链 28 条高校源进度任务；当前仍只作 double check，不确认字段事实、不写回、不替代湖北官方计划。
...
- 高校源 Adapter D0/D1 页列核验进度公开账本：`data/working/issue19-school-source-adapter-d0-d1-page-side-progress-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-page-side-progress-v1-summary.json`。它是公开状态机/进度账本，只同步每个页列私有 CSV 的人工填写进度：PDF 原页记录、湖北官方计划记录、高校源差异解释、最终字段处理建议、双人复核和字段写回门禁是否已填、是否可进入字段写回评审；不含学校名、专业名、字段值、OCR 正文、人工读数或私有路径。当前仍为 `not_final`，字段事实、推荐依据、最终可用、学校专业建议均为 0 或 `false`，只能回答“私有核验项有没有填、能否进入字段写回评审”。
- 高校源 Adapter D0/D1 页列 PDF 视觉核验审计：`data/working/issue19-school-source-adapter-d0-d1-page-side-pdf-visual-audit-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-page-side-pdf-visual-audit-v1-summary.json`。把 18 个 D0/D1 页列接到本地第 19 期整页图和左右栏裁图，覆盖 14 个 PDF 页、146 条私有核验项；公开层只保存页列、计数、尺寸、证据编号和 SHA，栏图、整页图、学校专业线索、OCR 线索、人工记录和本地路径只留在 Git 忽略私有目录。它只降低人工回看 PDF 原页的定位成本，不确认字段事实，不替代湖北官方计划。
- 高校源 Adapter D0/D1 逐项证据路由：`data/working/issue19-school-source-adapter-d0-d1-item-evidence-route-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-item-evidence-route-v1-summary.json`。把 146 条私有核验项逐条接到页列包、页列进度和 PDF 视觉审计，公开层只保存状态桶、证据编号和 SHA；学校、专业、代码、OCR 线索、字段读数和人工记录仍只在私有材料中。该表用于确认每一项还差 PDF 原页、湖北官方侧和高校辅证哪类证据，不确认字段事实，不进入推荐。
- 高校源 Adapter D0/D1 逐项准出门禁：`data/working/issue19-school-source-adapter-d0-d1-item-resolution-gate-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-item-resolution-gate-v1-summary.json`。把上述 146 条逐项证据路由转成字段写回前的缺口门禁：PDF 原页缺口 146、湖北官方侧缺口 146、高校辅证未验证 146、冲突处理和双人复核缺口各 29；全部仍阻断在私有写回评审之前，不确认字段事实，不进入推荐。
...
- 第一闭环事实准出门禁账本：`data/working/issue19-first-closure-fact-resolution-gate-v1-public-ledger.csv`、`data/working/issue19-first-closure-fact-resolution-gate-v1-page-summary.csv`、`data/working/issue19-first-closure-fact-resolution-gate-v1-task-summary.csv` 和 `data/working/issue19-first-closure-fact-resolution-gate-v1-summary.json`。把同一批 439 个事实范围逐条映射到“还缺哪类证据才能进入私有写回评审”：当前 PDF 原页待补 439、湖北官方侧待补 439、三方闭环待补 439、高校辅证待补 201、冲突待处理 275、双人复核待完成 146、专业名归属待闭环 48、专业组边界待闭环 37；可进入私有写回评审、字段写回、推荐依据、官网替代湖北官方计划和最终可用全部为 0。
- 第一闭环准出执行叠加表：`data/working/issue19-first-closure-resolution-execution-overlay-v1.csv` 和 `data/working/issue19-first-closure-resolution-execution-overlay-v1-summary.json`。把第一闭环执行队列、439 个事实准出缺口、核验结果页列汇总和公开证据地图按 37 个页列合并成下一轮补证入口：R0 W0/B0 冲突事实先闭环 10 个页列、R1 专业名归属事实先闭环 9 个页列、R2 双人复核事实先闭环 18 个页列；事实、任务、缺口和非最终门禁全部守恒，仍不确认字段事实、不写回、不生成学校专业建议。
- 第一闭环事实证据通道工作台：`data/working/issue19-first-closure-fact-evidence-channel-workbench-v1-public-ledger.csv` 和 `data/working/issue19-first-closure-fact-evidence-channel-workbench-v1-summary.json`。把 439 个事实范围逐项接到 PDF 原页、OCR、机器坐标、高校官网辅证、湖北官方侧、冲突处理、双人复核和三方闭环通道，用于安排下一步最小核验动作；它不确认字段事实，不写回主表，不生成学校/专业建议或志愿推荐。`同校高校源*` 字段是随事实行重复的同校上下文，不能跨行求和。
- 第一闭环事实动作包：`data/working/issue19-first-closure-fact-action-packets-v1-public-ledger.csv` 和 `data/working/issue19-first-closure-fact-action-packets-v1-summary.json`。把同一批 439 个事实范围压成 79 个 `页列×事实核验动作组` 执行包，覆盖 37 个页列和 A0/A1/A2/A3/A4/A6/A7 七类动作；它用于按批执行 PDF 原页、湖北官方侧、高校辅证、冲突处理、双人复核、专业名归属和专业组边界核验，不确认字段事实、不写回、不进入推荐。`同校高校源*` 在这里按高校源进度看板 ID 去重，只是同校上下文。
- 第一闭环事实动作包准出门禁：`data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-public-ledger.csv` 和 `data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-summary.json`。把 79 个动作包逐包判定为写回前门禁，当前全部为 `blocked_missing_required_evidence`；主缺口桶为冲突 10 包、专业组边界 27 包、双人复核 21 包、专业名归属 8 包、PDF/湖北官方 11 包、高校辅证 2 包。它只说明每个执行包还缺什么证据，不确认字段事实、不进入推荐。
- 第一闭环 G0 冲突动作包闭环工作台：`data/working/issue19-first-closure-g0-conflict-package-closure-workbench-v1-public-ledger.csv` 和 `data/working/issue19-first-closure-g0-conflict-package-closure-workbench-v1-summary.json`。把动作包准出门禁中的 10 个 G0 冲突包单独抽出，接到 W0/B0 最小人工复核、私有预填 SHA、高校源桥接、字段回接和准出执行叠加表：共覆盖 275 个待核事实、87 个最小人工复核事实、188 个同页伴生待核事实和 68 个 PDFOCR 与高校辅证冲突字段。当前 10 包全部仍为 `blocked_pending_pdf_hubei_conflict_double_review`，只用于安排 PDF 原页、湖北官方侧、冲突处理和双人复核，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段复核 Overlay：`data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-summary.json`。把上述 68 个 PDFOCR 与高校辅证冲突字段逐字段落成私有复核入口，字段分布为专业计划数 26、学费 26、再选科目 16，并按 10 个页列生成私有 CSV SHA。当前 68 行全部仍为 `R0-Overlay已生成未填写`，只能用于人工填写 PDF 原页记录、湖北官方记录、高校辅证核验和双人复核，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段准出门禁：`data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-summary.json`。把上述 68 个冲突字段从“已生成复核入口”继续落到写回前缺口门禁：PDF 原页、湖北官方记录、高校辅证、冲突处理、三方一致性、字段确认和写回评审缺口各 68，双人复核缺口 47；当前 68 行全部为 `blocked_missing_required_field_evidence`，0 行可进入私有写回评审，不确认字段事实、不写回、不作为志愿推荐依据。
- 第一闭环 G0 冲突字段补证执行包：`data/working/issue19-first-closure-g0-conflict-field-evidence-execution-packets-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-evidence-execution-items-v1-public-ledger.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-evidence-execution-packets-v1-summary.json`。把 68 个被准出门禁阻断的冲突字段压成 10 个页列补证包和 68 个逐字段补证执行项；当前 7 个页列为 P0 双人复核优先、1 个 P1、2 个 P2，68 个执行项全部仍待私有证据采集，0 行可写回、推荐或进入下一阶段。它只回答“下一步按哪个页列和字段补证”，不保存字段值、不确认招生计划事实。
- 第一闭环 G0 冲突字段补证闭环结果：`data/working/issue19-first-closure-g0-conflict-field-evidence-closure-result-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-evidence-closure-result-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-evidence-closure-result-v1-summary.json`。把 68 个逐字段补证执行项继续接到本地私有页列 CSV 的填写状态，只公开状态、计数、证据编号和 SHA：当前 PDF 原页、湖北官方侧、冲突处理、三方闭环、字段确认和写回评审仍各 68 个待闭环，双人复核待完成 47，0 行可进入私有写回评审；高校源私有种子线索 62 条只作为人工核验提示，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段补证缺口任务队列：`data/working/issue19-first-closure-g0-conflict-field-evidence-gap-tasks-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-evidence-gap-tasks-v1-page-channel-summary.csv`、`data/working/issue19-first-closure-g0-conflict-field-evidence-gap-tasks-v1-channel-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-evidence-gap-tasks-v1-summary.json`。把 68 个闭环结果拆成 523 条证据缺口任务：PDF 原页 68、湖北官方侧 68、高校辅证 68、冲突处理 68、双人复核 47、三方闭环 68、字段确认 68、写回评审 68；251 条当前可并行执行，272 条等待前置证据闭环。它只安排补证任务，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段补证执行波次：`data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-wave-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-summary.json`。把 523 条缺口任务压成 47 个 `页列×执行波次` 包和 5 个波次汇总：W0 一手证据与辅证采集 204 条、W1 双人复核 47 条、W2 冲突处理与三方闭环 136 条、W3 字段确认 68 条、W4 写回评审 68 条。W0/W1 共 251 条当前可并行推进，W2-W4 共 272 条等待前置闭环；仍不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段 W0/W1 主动工作板：`data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-summary.json`。把 W0/W1 当前可并行的 251 条任务压成 37 个 `页列×证据通道` 工作包和 10 个页列汇总：W0 204 条、W1 47 条；它只回答“现在先核哪些 PDF 原页、湖北官方侧、高校辅证和双人复核包”，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段 W0/W1 材料就绪审计：`data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-summary.json`。把 W0/W1 主动工作板后的开核材料状态收束为 37 个工作包、10 个页列；私有材料 37/37 就绪，人工记录 0/37 就绪，高校辅证 seed 62，状态为 `ready_not_final`。该审计只说明可以开核，不确认字段事实、不写回、不推荐、不进入最终方案。
- 第一闭环 G0 冲突字段 W0/W1 开核执行清单：`data/working/issue19-first-closure-g0-conflict-field-w0-w1-review-launch-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-w0-w1-review-launch-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-w0-w1-review-launch-v1-summary.json`。把材料已齐的 37 个工作包转成可分工的开核入口，10 个页列均为 `ready_to_launch_human_review_not_fact`；按通道统计，PDF 原页、湖北官方侧、高校辅证和 PDFOCR/高校辅证冲突提示均为 251 条任务通道记录，双人复核为 188 条任务通道记录。人工记录仍为 0/37，字段写回、推荐依据、学校专业建议、下一阶段和最终可用门禁全部为 0；它只安排开核动作，不确认字段事实、不写回、不进入最终方案。
- 第一闭环 G0 冲突字段 W0/W1 候选分层：`data/working/issue19-first-closure-g0-conflict-field-w0-w1-candidate-triage-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-conflict-field-w0-w1-candidate-triage-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-conflict-field-w0-w1-candidate-triage-v1-summary.json`。读取私有 Overlay 后只公开分层和计数：68 个唯一冲突字段中，30 个双侧线索冲突、10 个双侧线索一致但仍待核、22 个仅高校辅证、6 个仅 PDFOCR；按 W0/W1 任务通道展开为 251 条，其中冲突 112、一致 37、仅高校辅证 80、仅 PDFOCR 22。37 个开核包、10 个页列全部仍为 `candidate_conflict_ready_for_human_review_not_fact`，人工记录、字段确认、写回、推荐和最终可用均为 0。
- 第一闭环证据仲裁总视图：`data/working/issue19-first-closure-evidence-adjudication-board-v1-public-ledger.csv`、`data/working/issue19-first-closure-evidence-adjudication-board-v1-page-summary.csv` 和 `data/working/issue19-first-closure-evidence-adjudication-board-v1-summary.json`。把 206 条第一闭环任务和 37 个页列统一接到字段确认、G0 缺口任务、W0/W1 候选分层和高校源回接队列；其中 G0 命中 26 个任务、68 个字段事实、523 条证据通道，当前 251 条可并行、272 条待前置闭环。它只做证据准出排队，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 字段闭环进度表：`data/working/issue19-first-closure-g0-field-closure-progress-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-field-closure-progress-v1-page-summary.csv` 和 `data/working/issue19-first-closure-g0-field-closure-progress-v1-summary.json`。把 68 个 G0 冲突字段逐字段落到 10 个页列的进度账本：专业计划数 26、学费 26、再选科目 16；PDF 原页、湖北官方侧、高校辅证、冲突处理、三方闭环、字段确认和写回评审均仍待 68，双人复核仍待 47。它是后续填私有核验记录和解除阻断的主入口，不保存具体核验内容，也不作为志愿推荐依据。
- 第一闭环 G0 字段 W0/W1 可执行通道项：`data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-public-ledger.csv`、`data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-page-channel-summary.csv` 和 `data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-summary.json`。从 523 条 G0 缺口任务中只抽取当前可并行的 W0/W1 通道，形成 251 条 `字段事实×证据通道` 执行项和 37 个页列通道汇总：PDF 原页、湖北官方侧、高校辅证各 68，双人复核 47。它用于分派私有证据补录和解除阻断，不确认字段事实、不写回、不进入推荐。
...
- 第一闭环 W0/B0 高校源桥接账本：`data/working/issue19-w0-b0-school-source-bridge-public-ledger.csv`、`data/working/issue19-w0-b0-school-source-bridge-page-summary.csv` 和 `data/working/issue19-w0-b0-school-source-bridge-summary.json`。把 87 个核心事实接到高校源进度、最新证据对齐和结构化接入候选：68 条字段事实已有高校源可作 double check 提示，19 条专业名归属/专业组边界仍必须先核 PDF 原页和湖北官方侧；全部仍不确认字段事实、不替代湖北官方计划。
- 第一闭环 W0/B0 高校源字段回接队列：`data/working/issue19-w0-b0-school-source-field-backlink-queue-public-ledger.csv`、`data/working/issue19-w0-b0-school-source-field-backlink-page-summary.csv`、`data/working/issue19-w0-b0-school-source-field-backlink-school-summary.csv` 和 `data/working/issue19-w0-b0-school-source-field-backlink-summary.json`。把 68 条可 double check 的计划数/学费/选科字段事实进一步拆成 5 条 B1 结构化候选优先回接、45 条 B2 双人核页前回接、18 条 B2 普通冲突提示回接；它只服务私有核验材料回接，不确认字段值，不写回主表。
...
- 高校官网辅证进度看板：`data/working/issue19-school-source-progress-board-public-ledger.csv` 和 `data/working/issue19-school-source-progress-board-summary.json`。把 80 个高校侧辅证任务按最新证据层级、来源形态、留存状态和下一批动作重新排队；当前 L3/L1/L0 为 60/12/8，C4/C6 唯一口径仍是 411 条需补结构化、190 条需继续补源。
...
- 高校源 E0 人工回页队列：`data/working/issue19-school-source-e0-manual-page-review-queue-public-ledger.csv` 和 `data/working/issue19-school-source-e0-manual-page-review-queue-summary.json`。由 `scripts/build_issue19_school_source_e0_manual_page_review_queue.py` 从 37 条 E0 人工先核任务派生，35 条有同校第一闭环页列提示，2 条暂无同校页列提示；它只是桥接队列，不确认字段事实。
...
python3 scripts/build_issue19_first_closure_g0_conflict_field_resolution_gate_v1.py
python3 scripts/build_issue19_first_closure_g0_conflict_field_evidence_execution_packets_v1.py
python3 scripts/build_issue19_first_closure_g0_conflict_field_evidence_closure_result_v1.py
python3 scripts/build_issue19_first_closure_g0_conflict_field_evidence_gap_task_queue_v1.py
python3 scripts/build_issue19_first_closure_g0_conflict_field_evidence_gap_execution_waves_v1.py
...
第一闭环准出执行叠加表：`data/working/issue19-first-closure-resolution-execution-overlay-v1.csv` 和 `data/working/issue19-first-closure-resolution-execution-overlay-v1-summary.json` 不新增字段事实，而是把执行队列、事实准出门禁、核验结果页列汇总和公开证据地图叠到同一张页列入口。它保持 37 个页列、206 条任务、439 个事实范围、354 个字段事实、48 个专业名归属事实和 37 个专业组边界事实守恒，并把 37 个页列分成 R0 冲突先闭环 10 个、R1 专业名归属先闭环 9 个、R2 双人复核先闭环 18 个；所有写回、推荐、官网替代湖北官方计划、下一阶段和最终可用门禁仍为 0。

第一闭环事实证据通道工作台：`data/working/issue19-first-closure-fact-evidence-channel-workbench-v1-public-ledger.csv` 把上述 439 个事实范围从页列入口继续展开为事实级证据通道。事实域固定为字段事实 354、专业名归属 48、专业组边界 37；动作组包括 A0 W0/B0 冲突事实先核、A1 专业名归属先核、A2 专业组边界随页先核、A3 双人复核先核、A4 人工看图、A6 高校辅证提示回接和 A7 常规 PDF/湖北官方闭环。公开表不保存学校名、专业名、字段值、OCR 原文、图片路径、人工记录或登录态；所有最终可用、下一阶段、推荐依据、字段写回和官网替代湖北官方计划门禁均为 false 或 0。表内 `同校高校源*` 是行级上下文，不能直接纵向求和。

第一闭环事实动作包准出门禁：`data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-public-ledger.csv` 把 79 个 `页列×事实核验动作组` 执行包继续压成门禁账本。它逐包回链事实动作包、事实准出门禁账本和事实证据通道工作台，守恒 439 个事实、354 个字段事实、48 个专业名归属事实和 37 个专业组边界事实；所有包仍需 PDF 原页、湖北官方侧和三方闭环，字段写回、推荐依据、学校专业建议、下一阶段和最终可用门禁均为 false 或 0。
...
高校源 Adapter D0/D1 页列核验进度公开账本：`data/working/issue19-school-source-adapter-d0-d1-page-side-progress-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-page-side-progress-v1-summary.json` 是 D0/D1 页列私有 CSV 的公开状态机。它只公开每个页列的 PDF 原页记录、湖北官方计划记录、高校源差异解释、最终字段处理建议、双人复核和字段写回门禁的填写/评审状态，不保存学校名、专业名、字段值、OCR 正文、人工读数或私有路径。当前全部仍为 `not_final`，字段事实、推荐依据、最终可用、学校专业建议均为 0 或 `false`；该账本只回答私有核验项是否已填、是否能进入字段写回评审，不是可报结论。

高校源 Adapter D0/D1 页列 PDF 视觉核验审计：`data/working/issue19-school-source-adapter-d0-d1-page-side-pdf-visual-audit-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-page-side-pdf-visual-audit-v1-summary.json` 是 D0/D1 页列的本地视觉核验入口。它为 18 个页列生成私有栏图和审阅 HTML，并回链 14 个第 19 期源页图 SHA、18 个栏图 SHA、18 个私有 HTML SHA 和原页列 CSV/HTML SHA；公开层只保存这些证据编号、尺寸、计数和哈希，不公开图片、路径、学校专业明细、OCR 原文或人工读数。
