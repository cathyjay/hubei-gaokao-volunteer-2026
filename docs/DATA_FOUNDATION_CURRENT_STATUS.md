...
2. 按高校官网辅证 next20 和最新证据对齐账本继续自动补源和结构化；A4 继续补源任务中 4 条已有结构化或 diff 线索可转入核页提示，长春工业大学、浙江传媒学院、成都师范学院、韶关学院仍未取得可复用的 2026 湖北物理逐专业计划源或稳定解析入口。
3. 对 55 个重点核验组做完整组内专业接受度复核，当前 29 组建议进入下一轮重点核验。
