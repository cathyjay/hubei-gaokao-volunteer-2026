...
| `data/working/issue19-candidate-v3-b0-b1-official-crosscheck-queue.csv` | B0/B1 组级官方交叉校验队列 | 覆盖 49 个优先专业组；逐组记录 PDF、湖北官方系统、高校官网/章程、家庭接受度和调剂结论待核状态 |
| `data/working/issue19-candidate-v3-b0-b1-major-official-crosscheck-queue.csv` | B0/B1 逐专业官方交叉校验队列 | 覆盖 324 个逐专业任务；一行一个专业或 0 明细占位任务，逐专业保留计划数、学费、风险和字段待核状态 |
...
| B0/B1 可复用高校官网湖北计划源学校 | 1 |
| B0/B1 部分官网线索待跟进学校 | 3 |
| B0/B1 待补高校官网计划源学校 | 30 |
| B0/B1 官方交叉校验最终可用 | 0 |
| 专业行ID唯一数 | 13736 |
| 结构异常全量匹配 | 5129/5129 |
| 底座机器阻断项 | 0 |
...
22. **官方交叉校验三表**：B0/B1 拆成学校官方来源、组级官方交叉校验、逐专业官方交叉校验三层；高校官网只作辅助补证，若与湖北官方系统、省招办计划或志愿系统不一致，以湖北省招办渠道为准。
23. **0 明细占位保护**：`C10702`、`K15123` 只有 0 明细占位任务，占位任务只防止专业组丢失，不能冒充真实招生明细；真实逐专业任务当前为 322 条。
24. **机器校验**：`scripts/verify_baseline.py` 会检查全量明细行数、来源 SHA、候选命中数、候选工作台、优先/风险队列、页面复核包、完整性审计、候选 V2 逐专业明细、候选 V2 升级工作台、候选 V2 证据总账、全量质量分层、逐专业质量工作台、家庭底线筛选表、候选 V3 复核入口、候选 V3 B0/B1 核验包、B0/B1 官方交叉校验三表、底座审计、页级 manifest、`最终可用=false`、核验状态、0 明细占位、敏感路径和哈希清单。
...
- B0/B1 官方交叉校验三表中的 36 所学校、49 个专业组和 324 个逐专业任务；先补 30 所 `needs_official_plan_source_search` 学校的官网计划或章程来源，同时不能把 `has_reusable_2026_hubei_plan_source` 理解为最终通过。
