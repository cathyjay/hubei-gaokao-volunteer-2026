...
| `data/working/issue19-candidate-v3-b0-b1-admission-detail-official-crosscheck.csv` | B0/B1 逐专业招生明细主表 | 覆盖 324 个逐专业任务；把组级投档、调剂、历史线和补源字段下沉到每条专业行，后续候选讨论默认使用这张表 |
...
| `data/working/issue19-candidate-v3-b0-b1-official-evidence-match.csv` | B0/B1 逐专业官网证据匹配表 | 覆盖 324 条招生明细；一行一个专业，记录官网/API/PDF 抽取匹配状态、官网计划数和仍需核验项 |
| `data/working/issue19-b0-b1-retained-official-plan-normalized.csv` | B0/B1 留存官网证据标准化表 | 覆盖 332 条官网/API/HTML/XLSX/PDF 抽取证据；只作交叉校验来源，不是最终事实表 |
...
| B0/B1 部分官网线索待跟进学校 | 14 |
| B0/B1 待补高校官网计划源学校 | 10 |
| B0/B1 仅章程/规则或不可用计划线索学校 | 5 |
| B0/B1 留存官网/API/PDF 抽取证据标准化行 | 332 |
| B0/B1 逐专业官网证据专业名匹配 | 106 |
| B0/B1 逐专业计划数与 OCR 一致 | 42 |
| B0/B1 逐专业 OCR 缺失但官网可补计划数 | 52 |
| B0/B1 逐专业计划数存在差异 | 12 |
| B0/B1 官方交叉校验最终可用 | 0 |
| 专业行ID唯一数 | 13736 |
| 结构异常全量匹配 | 5129/5129 |
...
22. **官方交叉校验明细主表**：B0/B1 拆成学校官方来源、组级索引、逐专业招生明细主表、原逐专业核验队列和逐专业官网证据匹配表；候选讨论默认看逐专业招生明细，组级索引只用于投档、调剂、历史线和补源。高校官网只作辅助补证，若与湖北官方系统、省招办计划或志愿系统不一致，以湖北省招办渠道为准。
23. **0 明细占位保护**：`C10702`、`K15123` 只有 0 明细占位任务，占位任务只防止专业组丢失，不能冒充真实招生明细；真实逐专业任务当前为 322 条。
24. **机器校验**：`scripts/verify_baseline.py` 会检查全量明细行数、来源 SHA、候选命中数、候选工作台、优先/风险队列、页面复核包、完整性审计、候选 V2 逐专业明细、候选 V2 升级工作台、候选 V2 证据总账、全量质量分层、逐专业质量工作台、家庭底线筛选表、候选 V3 复核入口、候选 V3 B0/B1 核验包、B0/B1 逐专业招生明细主表、官方交叉校验工作台、逐专业官网证据匹配表、底座审计、页级 manifest、`最终可用=false`、核验状态、0 明细占位、敏感路径和哈希清单。
...
- B0/B1 逐专业招生明细主表中的 36 所学校、49 个专业组和 324 个逐专业任务；继续补 10 所 `needs_official_plan_source_search` 学校的官网计划或章程来源，同时不能把 `has_reusable_2026_hubei_plan_source` 理解为最终通过。
