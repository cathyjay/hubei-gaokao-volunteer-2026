...
24. **机器校验**：`scripts/verify_baseline.py` 会检查全量明细行数、来源 SHA、候选命中数、候选工作台、优先/风险队列、页面复核包、完整性审计、候选 V2 逐专业明细、候选 V2 升级工作台、候选 V2 证据总账、全量质量分层、逐专业质量工作台、家庭底线筛选表、候选 V3 复核入口、候选 V3 B0/B1 核验包、B0/B1 逐专业招生明细主表、官方交叉校验工作台、逐专业官网证据匹配表、逐专业证据合并表、底座审计、页级 manifest、`最终可用=false`、核验状态、0 明细占位、敏感路径和哈希清单。
