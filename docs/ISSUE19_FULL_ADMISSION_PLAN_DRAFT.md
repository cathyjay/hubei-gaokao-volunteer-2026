...
16. **机器校验**：`scripts/verify_baseline.py` 会检查全量明细行数、来源 SHA、候选命中数、候选工作台、优先/风险队列、页面复核包、完整性审计、候选 V2 逐专业明细、候选 V2 升级工作台、全量质量分层、逐专业质量工作台、底座审计、`最终可用=false`、核验状态、敏感路径和哈希清单。
17. **规则克制**：偏好专业标签只做关键词召回，不做最终专业分类；例如“师范相关”必须回看原 PDF 和专业目录确认。
18. **人工闸门**：进入最终志愿表前，必须回看第 19 期原 PDF 页，并与湖北官方平台或志愿系统、高校官网/招生章程交叉核验。
