...
65. **第 1 批样板复核**：`data/working/issue19-page-side-foundation-batch-01-sample-public-audit.csv` 先用一个高风险批次验证流程，覆盖 25 个页列、23 个 PDF 页、717 条专业明细和 2151 个字段任务。样板只输出分流和工作量，正式 Overlay 自动写回为 0；19 批并行时，每个 subagent 只允许处理自己的 batch 私有结果，公共总账和最终门禁必须统一汇总。
66. **规则克制**：偏好专业标签只做关键词召回，不做最终专业分类；例如“师范相关”必须回看原 PDF 和专业目录确认。
67. **人工闸门**：进入最终志愿表前，必须回看第 19 期原 PDF 页，并与湖北官方平台或志愿系统、高校官网/招生章程交叉核验。
