...
| `data/working/issue19-official-public-entry-status.json` | 官方公开入口状态快照 | 记录 2026-06-28 湖北教育考试网公开入口 `GET/200`、SHA、平台首页 SHA 和无登录 401 探针；说明当前公开入口尚不能替代逐专业核验 |
...
| `data/working/issue19-page-side-foundation-review-progress-public-ledger.csv` | 页列底座公开核页进度账本 | 把 19 批私有核页材料的填写状态同步成 462 个页列公开状态机 |
| `data/working/issue19-page-side-foundation-review-progress-public-ledger-summary.json` | 页列底座公开核页进度摘要 | 看 1441 个私有必填记录槽位、当前已填 0、推荐依据和最终可用计数均为 0 |
| `data/working/issue19-page-side-foundation-field-clue-public-audit.csv` | 页列底座字段线索公开审计 | 把 41208 个逐字段任务重新聚合到 462 个页列，只公开字段分布、状态桶、线索缺失/冲突计数和私有模板 SHA |
| `data/working/issue19-page-side-foundation-field-clue-public-audit-summary.json` | 页列底座字段线索审计摘要 | 看 29764 个有线索字段、11444 个缺线索字段、1137 个冲突/多值/疑似字段，以及全部待 PDF/湖北官方核验的门禁 |
| `data/working/issue19-page-side-foundation-human-review-overlay-public-ledger.csv` | 页列底座人工复核 Overlay 公开账本 | 公开同步 462 个页列的私有 Overlay 记录数、已填字段计数、私有 CSV SHA 和非最终门禁 |
| `data/working/issue19-page-side-foundation-human-review-overlay-public-ledger-summary.json` | 页列底座人工复核 Overlay 摘要 | 看 19 批、41208 条私有 Overlay 记录、缺失 0、人工填写 0、字段确认 0、最终可用 0 |
| `data/working/issue19-page-side-foundation-batch-01-sample-public-audit.csv` | 第 1 批样板复核公开审计 | 覆盖 25 个页列、717 条招生专业明细、2151 个字段任务，只公开计数、动作桶、私有详表 SHA 和非最终门禁 |
| `data/working/issue19-page-side-foundation-batch-01-sample-public-audit-summary.json` | 第 1 批样板复核摘要 | 看 Q0 831、Q1 1071、Q2 249，以及正式 Overlay 写回、推荐依据和最终可用均为 0 |
| `data/working/issue19-page-side-foundation-all-batch-review-public-ledger.csv` | 全 19 批复核公开账本 | 覆盖 462 个页列、13736 条招生专业明细、41208 个字段任务，只公开计数、状态分布、私有详表 SHA 和非最终门禁 |
...
| `data/working/issue19-field-fact-p0-reread-machine-candidates-summary.json` | P0 字段机器坐标候选摘要 | 看单一候选、重复同值候选、多值冲突、无候选、字段候选数量和全部禁止自动写回门禁 |
| `data/working/issue19-field-fact-p0-closure-action-workbench.csv` | P0 字段闭环推进工作台 | 覆盖同一批 11444 条 K0 字段任务；把机器候选、PDF 原页人工核页、湖北官方字段核验和高校官网/章程辅证分列，不把候选当事实 |
| `data/working/issue19-field-fact-p0-closure-action-workbench-summary.json` | P0 字段闭环推进摘要 | 看 A1/A1R/A2/A3 动作桶、三方待核状态、人工/官方字段确认计数和全部非最终门禁 |
| `data/working/issue19-field-fact-p0-semantic-crosssource-audit.csv` | P0 字段语义与多源线索审计表 | 覆盖同一批 11444 条 K0 字段任务；把机器候选语义异常、计划数偏大、高校官网/章程补缺线索和机器/高校辅证冲突下沉到字段任务 |
| `data/working/issue19-field-fact-p0-semantic-crosssource-audit-summary.json` | P0 字段语义与多源线索摘要 | 看语义异常 15 条、计划数偏大 11 条、机器/高校辅证一致 22 条、冲突 1 条、官网补缺线索 52 条和全部非最终门禁 |
| `data/working/issue19-field-fact-p0-triage-execution-workbench.csv` | P0 字段三方核验执行工作台 | 覆盖同一批 11444 条 K0 字段任务；把语义优先桶转成执行总序、执行批次和执行方式，并回连 PDF 原页锚点、湖北官方待核包和高校辅证线索 |
| `data/working/issue19-field-fact-p0-triage-execution-workbench-summary.json` | P0 字段三方核验执行摘要 | 看执行批次、双人复核 245 条、PDF 锚点命中 11444 条、湖北官方待核包命中 11444 条和全部非最终门禁 |
| `data/working/issue19-field-fact-p0-immediate-review-packet.csv` | P0 字段即时复核包 | 从 P0 字段三方核验执行工作台严格切出 `EXEC-01/02/03/04` 共 319 条最高优先级字段任务；仍是一行一个 `专业行ID × 字段名`，不写回事实 |
| `data/working/issue19-field-fact-p0-immediate-review-packet-summary.json` | P0 字段即时复核包摘要 | 看父表 11444 行、切片 319 行、执行包 222 个、PDF 页 114 个、院校代码 202 个、双人复核 245 条和全部非最终门禁 |
| `data/working/issue19-p0-immediate-pdf-crop-evidence-index.csv` | P0 即时复核裁图证据索引 | 覆盖同一批 319 条即时复核字段任务；公开表只保存裁图证据编号、裁图文件 SHA256、裁图规格 SHA256、页码、版面列、bbox、页图/窗口哈希和非最终门禁 |
| `data/working/issue19-p0-immediate-pdf-crop-evidence-index-summary.json` | P0 即时复核裁图证据摘要 | 看 319 条本地裁图证据、114 个 PDF 页、148 个页码×版面列组合、哈希计数和全部非最终门禁 |
| `data/working/issue19-p0-immediate-three-way-closure-public-ledger.csv` | P0 即时三方闭环公开账本 | 覆盖同一批 319 条即时复核字段任务；公开表只保存任务 ID、证据编号、SHA、bbox、三方状态和非最终门禁，不保存字段读数或最终确认值 |
| `data/working/issue19-p0-immediate-three-way-closure-public-ledger-summary.json` | P0 即时三方闭环公开账本摘要 | 看 319 条闭环任务、75 条高校字段线索、23 条机器/高校可比对线索、22 条一致、1 条冲突、52 条高校补缺线索和全部非最终门禁 |
| `data/working/issue19-p0-immediate-crop-ocr-public-audit.csv` | P0 即时裁图 OCR 公开审计表 | 覆盖同一批 319 条即时复核字段任务；公开表只保存 OCR 状态、候选关系、SHA、bbox、人工优先桶和非最终门禁，不保存识别文本、候选读数或图片路径 |
| `data/working/issue19-p0-immediate-crop-ocr-public-audit-summary.json` | P0 即时裁图 OCR 公开审计摘要 | 看 319 张裁图识别覆盖、253 条可比 OCR 候选、50 条冲突优先人工核页、35 条一致但仍待三方核验和 66 条未能稳定补读 |
| `data/working/issue19-p0-immediate-field-confirmation-public-ledger.csv` | P0 即时字段确认公开账本 | 覆盖同一批 319 条即时复核字段任务；公开表只保存私有字段确认状态、证据编号、SHA、bbox、关系状态和非最终门禁，不保存字段记录值、候选值、识别文本或图片路径 |
| `data/working/issue19-p0-immediate-field-confirmation-public-ledger-summary.json` | P0 即时字段确认公开账本摘要 | 看 319 条字段确认任务、75 条高校辅证私有记录待完成、290 条双人复核待完成、字段写回评估可进入数 0 和全部非最终门禁 |
| `data/working/issue19-p0-immediate-page-review-packets.csv` | P0 即时按页核页包 | 把 319 条字段确认任务按 `PDF页码×版面列` 聚合成 148 个执行包；公开表只保存任务数、证据编号、SHA、bbox 摘要、状态和非最终门禁 |
...
| P0 字段机器多值冲突 | 218 |
| P0 字段机器仍无候选 | 6386 |
| P0 字段机器候选：专业计划数 | 2175 |
| P0 字段机器候选：再选科目 | 1994 |
| P0 字段机器候选：学费 | 671 |
| P0 字段闭环推进任务 | 11444 |
| P0 字段闭环 A1/A1R 快速候选核页 | 4840 |
| P0 字段闭环 A2 多值冲突核页 | 218 |
| P0 字段闭环 A3 无候选重读 | 6386 |
| P0 字段闭环 PDF 人工确认 | 0 |
| P0 字段闭环湖北官方确认 | 0 |
| P0 字段闭环三方闭环 | 0 |
| P0 字段语义与多源线索任务 | 11444 |
| P0 字段机器候选语义异常 | 15 |
| P0 字段计划数偏大需重点核页 | 11 |
| P0 字段机器候选与高校辅证一致 | 22 |
| P0 字段机器候选与高校辅证冲突 | 1 |
...
| P0 字段三方核验执行包 | 2779 |
| P0 字段三方核验双人复核任务 | 245 |
| P0 字段三方核验无候选原页重读 | 6334 |
| P0 字段即时复核任务 | 319 |
| P0 字段即时复核执行包 | 222 |
| P0 字段即时复核覆盖 PDF 页 | 114 |
| P0 即时复核本地裁图证据 | 319 |
| P0 即时复核裁图覆盖页码×版面列 | 148 |
| P0 即时三方闭环公开账本任务 | 319 |
| P0 即时三方闭环高校字段线索 | 75 |
| P0 即时三方闭环机器/高校可比对 | 23 |
| P0 即时三方闭环机器/高校一致 | 22 |
| P0 即时三方闭环机器/高校冲突 | 1 |
| P0 即时三方闭环高校补缺线索 | 52 |
| P0 即时裁图 OCR 公开审计任务 | 319 |
| P0 即时裁图 OCR 有可比候选 | 253 |
| P0 即时裁图 OCR 冲突优先人工核页 | 50 |
| P0 即时裁图 OCR 与既有线索一致仍待核验 | 35 |
| P0 即时裁图 OCR 未能稳定补读需人工看图 | 66 |
| P0 即时字段确认公开账本任务 | 319 |
| P0 即时字段确认需高校辅证私有记录 | 75 |
| P0 即时字段确认需双人复核 | 290 |
| P0 即时字段确认可进入写回评估 | 0 |
| P0 即时按页核页包 | 148 |
| P0 即时按页核页覆盖 PDF 页 | 114 |
| P0 即时按页核页字段任务合计 | 319 |
| P0 即时按页核页裁图证据合计 | 319 |
| P0 即时按页核页双人复核任务 | 290 |
| P0 即时按页核页可进入写回评估 | 0 |
| 页列底座人工复核 Overlay 公开页列 | 462 |
| 页列底座人工复核 Overlay 私有字段记录 | 41208 |
| 页列底座人工复核 Overlay 缺失记录 | 0 |
| 页列底座人工复核 Overlay 已填人工记录 | 0 |
| 第 1 批样板复核页列 | 25 |
| 第 1 批样板复核 PDF 页 | 23 |
| 第 1 批样板复核专业明细 | 717 |
| 第 1 批样板复核字段任务 | 2151 |
| 第 1 批样板 Q0 无候选阻断字段 | 831 |
| 第 1 批样板 Q1 带候选待核字段 | 1071 |
| 第 1 批样板 Q2 待三方闭环字段 | 249 |
| 第 1 批样板正式 Overlay 自动写回 | 0 |
| 全 19 批复核页列 | 462 |
| 全 19 批复核 PDF 页 | 231 |
| 全 19 批复核专业明细 | 13736 |
| 全 19 批复核字段任务 | 41208 |
| 全 19 批 Q0 无候选阻断字段 | 15813 |
| 全 19 批 Q1 带候选待核字段 | 21606 |
| 全 19 批 Q2 待三方闭环字段 | 3789 |
| 全 19 批人工填写/推荐依据/最终可用 | 0 |
| 逐专业证据路由总行数 | 13736 |
| 逐专业证据路由 L3 高校辅证 | 854 |
| 逐专业证据路由 L4 OCR 或单源线索 | 12882 |
| 逐专业证据路由 P0 100% 人工核验 | 5043 |
| 逐专业证据路由 P1 页列集中核验 | 7952 |
| 逐专业证据路由 P2 自动官网核验后人工确认 | 557 |
...
issue19_field_fact_p0_reread_machine_candidates_not_final
```
...
47. **官方公开入口状态快照**：`data/working/issue19-official-public-entry-status.json` 记录 2026-06-28 湖北教育考试网招生计划页面和索引页公开 HTTP 复核状态，计划页 SHA 为 `5c56b9582418af6e1cfbd40431920a0fee28807492c6be30b972d118251e8776` 且仍含“持续更新中/敬请期待”；湖北招生数智平台首页 SHA 为 `6dade2ef84ab249dd9d700a24e98c63f40f8305bba2bb6250acbf8cf10fcaba3`，无登录探针仍为 401。该快照只说明当前官方公开入口边界，不能替代第 19 期逐专业明细、湖北官方系统字段核验和高校章程交叉核验。
...
52. **P0 字段机器坐标候选**：`data/working/issue19-field-fact-p0-reread-machine-candidates.csv` 在 11444 条 K0 字段任务上生成 4840 条非空坐标候选、6386 条无候选和 218 条多值冲突。该表公开输出只保存候选值、坐标摘要、必要来源 ID、页码/版面列、字段名、证据编号、哈希和非最终门禁，不保存 OCR 窗口原文、院校名、专业名、专业代号或专业组代码等上下文字段；所有候选都必须回看 PDF 原页并用湖北官方系统或省招办计划确认，不能自动写回主表、不能回填候选、不能生成学校或专业建议。
53. **P0 字段闭环推进**：`data/working/issue19-field-fact-p0-closure-action-workbench.csv` 把同一批 11444 条 K0 字段任务转成 A1/A1R 快速候选核页、A2 多值冲突核页和 A3 无候选重读三类执行批次，并把机器候选字段值、PDF 原页人工读数、湖北官方字段值、高校官网或章程字段值分列。当前 PDF 人工确认、湖北官方确认、高校辅证确认和三方闭环计数均为 0。
54. **P0 字段语义与多源线索审计**：`data/working/issue19-field-fact-p0-semantic-crosssource-audit.csv` 在 P0 字段闭环推进工作台上增加机器候选语义范围检查和 B0/B1 高校官网/章程字段线索。它把 15 条语义异常、11 条计划数偏大、1 条机器/高校辅证冲突、22 条机器/高校辅证一致和 52 条高校补缺线索排到不同优先桶；这些优先桶只安排核 PDF 原页和湖北官方系统，不写回字段事实。
55. **P0 字段三方核验执行工作台**：`data/working/issue19-field-fact-p0-triage-execution-workbench.csv` 把同一批 11444 条字段任务排成可执行顺序，并按冲突异常、计划数偏大、高校辅证线索、多值坐标、常规机器候选和无候选原页重读分批。它只做核验派单和证据回链，不得把执行包、机器候选或高校辅证线索当作字段事实。
56. **P0 字段即时复核包**：`data/working/issue19-field-fact-p0-immediate-review-packet.csv` 是父表的严格切片，只包含 `EXEC-01/02/03/04` 的 319 条字段任务。它用于先核最容易影响字段事实的冲突、异常、官网辅证线索和多值坐标问题；即使机器候选与高校辅证一致，也仍必须核 PDF 原页和湖北官方系统，不能直接写回。
57. **P0 即时复核裁图证据索引**：`data/working/issue19-p0-immediate-pdf-crop-evidence-index.csv` 为 319 条即时复核字段任务生成本地 PDF 原页裁图，并公开保存证据编号、哈希和 bbox。它只服务人工回看和证据复验，不提供字段读数，不写回字段事实。
58. **P0 即时三方闭环公开账本**：`data/working/issue19-p0-immediate-three-way-closure-public-ledger.csv` 严格继承同一批 319 条任务，把 PDF 原页、湖北官方系统或省招办计划、高校官网或招生章程三条证据链的私有核验状态分开记录。当前 75 条高校字段线索只能用于安排核验顺序，其中 22 条机器/高校一致、1 条冲突、52 条高校补缺线索均不得绕过 PDF 原页和湖北官方核验；公开账本不保存字段读数或最终确认值。
59. **P0 即时裁图 OCR 公开审计**：`data/working/issue19-p0-immediate-crop-ocr-public-audit.csv` 用 Apple Vision OCR 对 319 张私有裁图做第二次机器复读，并公开保存状态和关系。它可以把 50 条冲突优先人工核页、35 条一致但仍待三方核验和 66 条未能稳定补读的任务排出来，但不保存识别文本或读数，也不能替代 PDF 原页人工核页。
60. **P0 即时字段确认公开账本**：`data/working/issue19-p0-immediate-field-confirmation-public-ledger.csv` 把同一批 319 条任务从“派单和裁图复读”推进到“私有人工记录状态机”。公开账本只同步 PDF 原页、湖北官方、高校辅证和双人复核的完成状态，不同步字段记录值；当前 319 条仍待 PDF 原页和湖北官方私有记录，75 条仍待高校辅证私有记录，290 条仍待双人复核，字段写回评估可进入数为 0。
61. **P0 即时按页核页包**：`data/working/issue19-p0-immediate-page-review-packets.csv` 把 319 条字段确认任务按 `PDF页码×版面列` 聚合成 148 个执行包，覆盖 114 个 PDF 页。公开表只保存任务数、证据编号集合、SHA、bbox 摘要和门禁；私有 HTML/CSV 才显示裁图和候选线索。它用于人工逐页逐列核 PDF 原页，防止左右列串读，不生成字段事实、目标院校或最终志愿方案。
62. **页列底座公开核页进度账本**：`data/working/issue19-page-side-foundation-review-progress-public-ledger.csv` 覆盖 19 批、462 个页列、231 个 PDF 页、13736 条专业明细和 41208 个字段任务，把私有批次核页材料中的填写状态同步为公开状态机。当前 1441 个必填记录槽位全部未填，PDF 原页、湖北官方、结构/官方消歧、高校辅证、升级条件、推荐依据和最终可用计数均为 0；它只说明核页进度，不说明字段已核准。
63. **页列底座字段线索公开审计**：`data/working/issue19-page-side-foundation-field-clue-public-audit.csv` 覆盖同一批 462 个页列和 41208 个字段任务，把字段任务回链、三字段分布、P/K/Q 状态桶、线索缺失、冲突/多值/疑似线索、PDF/湖北官方待核状态和私有逐字段线索模板 SHA 放在公开层。当前 29764 个字段任务有线索、11444 个缺线索、1137 个有冲突/多值/疑似信号，但字段写回、推荐依据和最终可用计数仍为 0；该表只说明“该核什么、缺什么”，不说明字段已核准。
64. **页列底座人工复核 Overlay**：`data/working/issue19-page-side-foundation-human-review-overlay-public-ledger.csv` 在不可变字段线索模板之上建立私有人工复核层，公开表只保存 462 个页列的计数、SHA 和非最终门禁；私有 Overlay 覆盖 41208 个字段任务，后续 PDF 原页人工读数、湖北官方字段值、高校官网或章程辅证值、字段确认值和复核结论只写入这里，不覆盖机器底稿。
