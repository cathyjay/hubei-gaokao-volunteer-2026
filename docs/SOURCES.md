...
- `data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-public-ledger.csv`：第一闭环事实动作包准出门禁，79 行；一行对应一个 `页列×事实核验动作组` 执行包，公开展示该包进入私有写回评审前仍缺的 PDF 原页、湖北官方侧、高校辅证、冲突处理、双人复核、三方闭环、专业名归属和专业组边界证据。当前全部阻断，不确认字段事实。
- `data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-summary.json`：第一闭环事实动作包准出摘要；记录 79 个动作包、37 个页列、439 个事实，以及冲突、专业组边界、双人复核、专业名归属、PDF/湖北官方和高校辅证六类主缺口桶分布；所有写回、推荐和最终门禁均为 0。
- `data/working/issue19-first-closure-g0-conflict-package-closure-workbench-v1-public-ledger.csv`：第一闭环 G0 冲突动作包闭环工作台，10 行；一行对应一个 G0 冲突动作包，回链 W0/B0 最小人工复核、执行预填审计、高校源桥接、字段回接和准出执行叠加表，只保存状态、计数、证据编号和 SHA。
- `data/working/issue19-first-closure-g0-conflict-package-closure-workbench-v1-summary.json`：第一闭环 G0 冲突动作包闭环工作台摘要；记录 10 个冲突包、275 个待核事实、87 个最小人工复核事实、188 个同页伴生待核事实、68 个冲突字段和 10 个私有材料已生成包；所有写回、推荐、官网替代湖北官方计划、下一阶段和最终可用门禁均为 0。
- `data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-public-ledger.csv`：第一闭环 G0 冲突字段复核 Overlay 公开账本，68 行；一行对应一个 PDFOCR 与高校辅证冲突字段，回链 G0 冲突动作包、W0/B0 执行预填明细和高校源字段回接队列。公开层只保存 ID、状态、计数和私有记录 SHA，不保存字段读数、候选值、学校专业明细、OCR 原文或人工记录。
- `data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-page-summary.csv`：第一闭环 G0 冲突字段复核 Overlay 页列汇总，10 行；按 `PDF页码×版面列` 汇总 68 个冲突字段、26 个任务、26 条专业行、47 个双人复核字段、68 个人工看图字段和 5 个结构化接入候选字段。
- `data/working/issue19-first-closure-g0-conflict-field-review-overlay-v1-summary.json`：第一闭环 G0 冲突字段复核 Overlay 摘要；记录主表 68 行、页列 10 行、私有主表和私有索引 SHA、字段分布 26/26/16，以及所有写回、推荐、官网替代湖北官方计划、下一阶段和最终可用门禁均为 0。
- `data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-public-ledger.csv`：第一闭环 G0 冲突字段准出门禁公开账本，68 行；读取 G0 冲突字段复核 Overlay 和本地未公开复核材料，只公开每个冲突字段进入私有写回评审前的缺口状态、证据编号和 SHA。
- `data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-page-summary.csv`：第一闭环 G0 冲突字段准出门禁页列汇总，10 行；按 `PDF页码×版面列` 汇总 68 个冲突字段的 PDF 原页、湖北官方、高校辅证、冲突处理、双人复核、三方一致性、字段确认和写回评审缺口。
- `data/working/issue19-first-closure-g0-conflict-field-resolution-gate-v1-summary.json`：第一闭环 G0 冲突字段准出门禁摘要；记录 68 行字段、10 行页列、上游 Overlay SHA、私有 Overlay SHA、当前全部阻断和所有推荐/写回/最终门禁为 0 的边界。
- `data/working/issue19-first-closure-g0-conflict-field-evidence-execution-packets-v1-public-ledger.csv`：第一闭环 G0 冲突字段补证执行包公开账本，10 行；按 `PDF页码×版面列` 把 68 个被准出门禁阻断的冲突字段压成下一轮补证包，公开层只保存执行优先级、缺口计数、证据编号和 SHA。
- `data/working/issue19-first-closure-g0-conflict-field-evidence-execution-items-v1-public-ledger.csv`：第一闭环 G0 冲突字段补证执行项公开账本，68 行；一行对应一个冲突字段补证执行项，逐项回链 G0 冲突字段准出门禁和复核 Overlay，不公开字段值、候选值、学校专业明细、OCR 原文或人工记录。
- `data/working/issue19-first-closure-g0-conflict-field-evidence-execution-packets-v1-summary.json`：第一闭环 G0 冲突字段补证执行包摘要；记录 10 个页列补证包、68 个逐字段执行项、字段分布 26/26/16、P0/P1/P2 页列分布 7/1/2，以及所有推荐/写回/最终门禁为 0 的边界。
...
- `data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-public-ledger.csv`：第一闭环 G0 冲突字段补证执行波次公开账本，47 行；一行对应一个 `页列×执行波次`，把 523 条缺口任务压成 W0-W4 五个执行波次。
- `data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-wave-summary.csv`：第一闭环 G0 冲突字段补证执行波次汇总，5 行；记录 W0 204、W1 47、W2 136、W3 68、W4 68 的任务分布，W0/W1 当前可并行推进，W2-W4 等待前置闭环。
- `data/working/issue19-first-closure-g0-conflict-field-evidence-gap-execution-waves-v1-summary.json`：第一闭环 G0 冲突字段补证执行波次摘要；记录 47 个波次包、5 个波次汇总、523 个来源缺口任务、251 个当前可并行任务和所有写回、推荐、下一阶段、最终门禁为 0 的边界。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-public-ledger.csv`：第一闭环 G0 冲突字段 W0/W1 主动工作板公开账本，37 行；一行对应一个 `页列×证据通道`，只抽取 W0/W1 当前可并行的 PDF 原页、湖北官方侧、高校辅证和双人复核任务。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-page-summary.csv`：第一闭环 G0 冲突字段 W0/W1 主动工作板页列汇总，10 行；按页列汇总 251 条当前可并行任务、37 个工作包和 W0/W1 分布。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-active-workboard-v1-summary.json`：第一闭环 G0 冲突字段 W0/W1 主动工作板摘要；记录工作包 37 行、页列 10 行、W0 204、W1 47、通道 01/02/03/05 分布和所有写回、推荐、下一阶段、最终门禁为 0 的边界。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-public-ledger.csv`：第一闭环 G0 冲突字段 W0/W1 材料就绪审计公开账本，37 行；一行对应一个开核工作包，只保存材料就绪、人工记录就绪、计数、ID 和 SHA。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-page-summary.csv`：第一闭环 G0 冲突字段 W0/W1 材料就绪审计页列汇总，10 行；按页列汇总材料就绪工作包、当前可并行任务、人工记录缺口和 seed 计数。
- `data/working/issue19-first-closure-g0-conflict-field-w0-w1-material-readiness-v1-summary.json`：第一闭环 G0 冲突字段 W0/W1 材料就绪审计摘要；记录 37 个工作包、10 个页列、私有材料 37/37 就绪、人工记录 0/37 就绪、高校辅证 seed 62 和 `ready_not_final` 状态。
...
- `data/working/issue19-first-closure-evidence-adjudication-board-v1-public-ledger.csv`：第一闭环证据仲裁总视图公开账本，206 行；一行对应一个第一闭环任务，把字段确认、G0 缺口任务、W0/W1 候选分层和高校源回接状态合并到证据准出视图。
- `data/working/issue19-first-closure-evidence-adjudication-board-v1-page-summary.csv`：第一闭环证据仲裁总视图页列汇总，37 行；按页列汇总 206 个第一闭环任务、G0 命中任务、68 个 G0 字段事实和 523 条证据通道。
- `data/working/issue19-first-closure-evidence-adjudication-board-v1-summary.json`：第一闭环证据仲裁总视图摘要；记录 G0 命中 26 个任务、68 个字段事实、251 条当前可并行通道、272 条前置阻断通道，以及所有写回、推荐、下一阶段和最终门禁为 0 的边界。
- `data/working/issue19-first-closure-g0-field-closure-progress-v1-public-ledger.csv`：第一闭环 G0 字段闭环进度公开账本，68 行；一行对应一个 G0 冲突字段事实，逐项回链准出门禁、闭环结果、缺口任务、W0/W1 开核执行清单、候选分层、高校源字段回接和字段确认账本。
- `data/working/issue19-first-closure-g0-field-closure-progress-v1-page-summary.csv`：第一闭环 G0 字段闭环进度页列汇总，10 行；按页列汇总 68 个字段事实、523 条证据通道、251 条当前可并行通道和 272 条前置阻断通道。
- `data/working/issue19-first-closure-g0-field-closure-progress-v1-summary.json`：第一闭环 G0 字段闭环进度摘要；记录字段分布 26/26/16、候选分层 30/10/6/22、字段状态 7/28/33、结构化接入候选 5 和高校源可 double check 68；所有写回、推荐和最终门禁保持为 0。
- `data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-public-ledger.csv`：第一闭环 G0 字段 W0/W1 可执行通道项公开账本，251 行；一行对应一个当前可并行的 `G0字段事实×证据通道`，只保存通道、状态、动作组、解除阻断条件、ID 和 SHA。
- `data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-page-channel-summary.csv`：第一闭环 G0 字段 W0/W1 页列通道汇总，37 行；按 `PDF页码×版面列×证据通道` 汇总 251 条可执行通道项。
- `data/working/issue19-first-closure-g0-field-w0-w1-action-items-v1-summary.json`：第一闭环 G0 字段 W0/W1 可执行通道项摘要；记录 68 个字段事实、26 个任务、10 个页列、37 个页列通道、W0 204 条、W1 47 条和所有写回、推荐、下一阶段、最终门禁为 0 的边界。
...
- `scripts/build_issue19_school_source_adapter_d0_d1_page_side_packets_v1.py`：读取 D0/D1 人工核验包公开账本和 146 条私有核验项，生成 18 行 `PDF页码×版面列` 页列人工核验包，并在 Git 忽略私有目录写出每页列 CSV/HTML 和私有索引；公开层只保存页列、计数、集合 SHA、私有材料 SHA 和非最终门禁。
- `scripts/build_issue19_school_source_adapter_d0_d1_item_evidence_route_v1.py`：读取 146 条 D0/D1 私有核验项、页列包、页列进度和 PDF 视觉审计，生成逐项证据路由公开账本；公开层只保存状态桶、证据编号、SHA 和非最终门禁，不保存学校、专业、代码、OCR 线索、字段读数或人工记录。
- `scripts/build_issue19_school_source_adapter_d0_d1_item_resolution_gate_v1.py`：读取 D0/D1 逐项证据路由公开账本，生成 146 行逐项准出门禁；公开层只保存缺口桶、门禁状态、证据编号、SHA 和非最终门禁，不保存学校、专业、代码、OCR 线索、字段内容或人工记录。
...
- `scripts/build_issue19_first_closure_fact_action_packet_resolution_gate_v1.py`：生成 `data/working/issue19-first-closure-fact-action-packet-resolution-gate-v1-public-ledger.csv` 和 summary，将 79 个事实动作包逐包转成准出门禁；公开层只保存状态、缺口桶、通道分布和 SHA，不保存学校名、专业名、字段值、OCR 原文、图片路径、人工记录或登录态。
- `scripts/build_issue19_first_closure_g0_conflict_package_closure_workbench_v1.py`：生成 `data/working/issue19-first-closure-g0-conflict-package-closure-workbench-v1-public-ledger.csv` 和 summary，将 10 个 G0 冲突动作包接到 W0/B0 最小复核、私有预填 SHA、高校源桥接、字段回接和准出执行叠加表；公开层只保存状态、计数、证据编号和 SHA，不保存学校名、专业名、字段值、候选值、OCR 原文、图片路径、人工记录或登录态。
- `scripts/build_issue19_first_closure_g0_conflict_field_review_overlay_v1.py`：生成 68 行 G0 冲突字段复核 Overlay 公开账本、10 行页列汇总和 Git 忽略的私有字段复核 CSV；用于把 PDFOCR 与高校辅证冲突字段交给人工填写 PDF 原页记录、湖北官方记录、高校辅证核验、双人复核和三方一致性，公开层只保留状态、计数和 SHA。
...
- `scripts/build_issue19_first_closure_g0_conflict_field_evidence_gap_execution_waves_v1.py`：生成 47 行 G0 冲突字段补证执行波次、5 行波次汇总和 summary；用于把 523 条缺口任务压成 W0-W4 页列执行包，方便按页列补 PDF 原页、湖北官方侧、高校辅证、双人复核、冲突闭环和写回评审，不确认字段事实。
...
- `scripts/build_issue19_first_closure_evidence_adjudication_board_v1.py`：生成 206 行第一闭环证据仲裁总视图、37 行页列汇总和 summary；用于把第一闭环任务、字段确认、G0 缺口任务、W0/W1 候选分层和高校源回接状态统一成证据准出看板，不确认字段事实、不写回、不进入推荐。
- `scripts/build_issue19_first_closure_g0_field_closure_progress_v1.py`：生成 68 行 G0 字段闭环进度表、10 行页列汇总和 summary；用于逐字段追踪 PDF 原页、湖北官方侧、高校辅证、冲突处理、双人复核、三方闭环、字段确认和写回评审缺口，不保存私有核验内容或最终结论。
- `scripts/build_issue19_first_closure_g0_field_w0_w1_action_items_v1.py`：生成 251 行 G0 字段 W0/W1 可执行通道项、37 行页列通道汇总和 summary；用于把 523 条缺口任务中的当前可并行通道转成可分派的私有证据补录动作，不确认字段事实、不写回、不进入推荐。
...
- `data/working/issue19-school-source-adapter-d0-d1-item-evidence-route-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-item-evidence-route-v1-summary.json`：高校源 Adapter D0/D1 逐项证据路由，把 146 条私有核验项逐条回链页列包、页列进度和 PDF 视觉审计；公开层只保存状态桶、证据编号和 SHA，不确认字段事实。
- `data/working/issue19-school-source-adapter-d0-d1-item-resolution-gate-v1-public-ledger.csv` 和 `data/working/issue19-school-source-adapter-d0-d1-item-resolution-gate-v1-summary.json`：高校源 Adapter D0/D1 逐项准出门禁，把 146 条路由逐条转成写回前缺口判断；当前全部阻断，不确认字段事实。
