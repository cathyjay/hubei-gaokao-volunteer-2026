...
新增稳定基座筛选视图：`data/working/issue19-stable-foundation-major-screening-view.csv` 覆盖 13736 条逐专业招生明细，`data/working/issue19-stable-foundation-group-screening-view.csv` 覆盖 3329 个院校专业组。它把家庭底线、教育部学校属性、字段事实闭环、证据路由、三年投档线索和整组调剂风险接到同一个入口；当前 678 条逐专业明细仅可作机器初筛线索，1666 个专业组进入机器观察池，所有最终门禁仍为 0。
...
新增高校侧辅证刷新公开账本：`data/working/issue19-stable-foundation-school-source-refresh-public-ledger.csv` 覆盖 36 所学校、80 个 `高校×高校侧辅证动作` 任务，把 854 条 B0/B1 高校辅证压缩成可自动复跑、补结构化、继续补源、分层抽检和少量人工确认的工作队列。S0/S1 任务必须回 PDF 原页和湖北官方侧，S3 强辅证只允许抽检，抽检失败立即升级同页列、同校或同专业组 100% 核验；公开层不保存字段读数、登录态或最终结论。

新增 C4/C6 高校源刷新执行包：`data/working/issue19-c4-c6-school-source-refresh-execution-packets.csv` 把学校刷新账本中最大的自动化缺口单独拆出，覆盖 36 个执行包、30 所学校和 601 条私有逐专业执行明细。其中 C4 411 条负责补湖北物理普通本科计划行结构化，C6 190 条负责继续寻找或获取高校官网 2026 湖北计划源。执行包按 X0/X1/X2/X3 分泳道，后续可以并行补源和解析；但它仍只生成来源 diff 和候选线索，不确认字段事实、不写回主表、不进入学校专业建议。

新增 C4/C6 已留存官网源复用审计：`data/working/issue19-c4-c6-retained-source-reuse-public-ledger.csv` 把 36 个执行包同已留存官网标准化证据做第一轮自动匹配，筛出 17 个已有可复用官网源的包和 206 条可生成候选 diff 的私有明细。下一步应优先处理 R0/R1 包，再把北京语言大学等新增 C6 原始源接入结构化 diff；所有候选仍需回 PDF 原页和湖北官方侧。
...
新增西安建筑科技大学 C4/C6 官方 API 源留存：`data/external/issue19-c4-c6-official-sources/xauat-2026-hubei-physics-normal.json` 和 `data/working/issue19-c4-c6-xauat-official-source-fetch-public-ledger.csv` 已证明该校 2026 湖北物理类普通类计划可无登录抓取。该源有 20 条逐专业计划、合计 80 人，可核专业名、计划数、校内专业组和选科；但缺湖北院校专业组代码、学费、专业代号和校区，仍只能作高校侧辅证。
...
新增官方不可得抽样门禁：`data/working/issue19-official-unavailable-sampling-gates.csv` 把“高校官网 double check + 最小人工核验”落成 105 行门禁：80 条高校侧任务继续负责自动复跑、补结构化、补源和强辅证抽检；25 条 P3 低风险专业明细用于抽样验收。C0/C1/C7 共 105 条明细必须 100% 核第 19 期原页，C2 强辅证抽检最低覆盖 25 条，抽检失败升级同页列、同校或同组 100% 核验。它只能压缩人工工作量，不能打开字段写回、推荐依据或最终可用门禁。

新增官方不可得抽样执行明细：`data/working/issue19-official-unavailable-sampling-execution-detail.csv` 把上述 105 行门禁展开成 155 条逐专业执行明细。后续人工不需要从 854 条高校辅证或 13736 条全量明细里反复找起点，而是先核 105 条高风险 100% 明细，再核 25 条 C2 强辅证抽样明细和 25 条 P3 低风险抽样明细。该表只负责把任务落到 `专业行ID`，仍不能确认字段、写回主表或生成学校专业建议。

新增官方不可得抽样复核 Overlay 公开账本：`data/working/issue19-official-unavailable-sampling-review-overlay-public-ledger.csv` 把这 155 条明细接到本地私有复核表。人工后续只需要在私有表里补 PDF 原页读数、湖北官方侧值、必要高校辅证、三方一致性和抽检失败/升级结论；公开账本只同步 SHA、状态和计数。当前 155 条全部 R0 未填写，字段写回、推荐依据、学校专业建议和最终可用仍全部为 0。

新增官方不可得抽样页列核验包：`data/working/issue19-official-unavailable-sampling-review-packets-public-ledger.csv` 把上述 155 条抽样复核明细压缩成 46 个 `PDF页码×版面列` 私有核页包，覆盖 40 个 PDF 页。后续人工打开本地 HTML/CSV 后，按页图和 OCR 行把 PDF 原页、湖北官方侧、高校辅证和三方一致性写回私有 Overlay；公开账本只保留页列计数、证据编号、SHA 和 R0 状态。这个动作解决的是“人工怎么少量、准确、可复核地核”，不解决“字段已经核准”。

新增官方不可得抽样页列执行队列：`data/working/issue19-official-unavailable-sampling-review-execution-queue.csv` 把 46 个页列核验包排成可执行顺序：先核 E0 冲突/错位 7 个和 E1 官网未匹配 11 个，再核 E2 官网补缺 3 个、E3 强辅证抽检 2 个、E4 低风险抽检 23 个。该队列让后续人工不需要在私有材料里自行判断先后顺序，但仍只安排核验，不确认字段、不写回主表、不生成学校专业建议。

新增官方不可得抽样私有预填公开审计：`data/working/issue19-official-unavailable-sampling-triage-prefill-public-audit.csv` 把 46 个页列执行队列接到 Git 忽略的私有预填工作台。私有层预填 155 条抽样明细中的高校官网专业名、计划数、学费、选科和 OCR 线索；公开层只保留页列级计数、17 个高校来源文件的聚合计数、私有 CSV SHA 和非最终门禁。它解决的是“人工核页时少翻表、少找官网”，不解决“字段已经核准”；PDF 原页、湖北官方侧和必要高校辅证仍必须人工闭环。

新增第一闭环批次包：`data/working/issue19-stable-foundation-first-closure-detail-packet.csv` 覆盖 206 条最高优先级明细任务，`data/working/issue19-stable-foundation-first-closure-page-side-packet.csv` 把它们压缩为 37 个页列、32 个 PDF 页。这个包优先处理 C0 冲突、C1 官网补缺、C7 官网未匹配、EXEC-01 冲突异常、EXEC-02 计划数偏大和 EXEC-03 高校辅证线索，目的是先把最可能影响底座可信度的页列集中核准；全部门禁仍为 false。
...
新增页列底座批次执行包：`data/working/issue19-page-side-foundation-batch-execution-packets.csv` 把 19 个页列底座核验批次提升为批次级执行入口，并在本地私有目录生成 19 份批次 HTML/CSV 核页材料。公开表只保存批次计数、私有材料证据编号和 SHA、状态和非最终门禁；当前 19 批全部 R0，PDF 原页核页、湖北官方核验、结构/官方消歧和高校辅证完成页列数全部为 0。

新增页列底座字段线索公开审计：`data/working/issue19-page-side-foundation-field-clue-public-audit.csv` 把 462 个页列与 41208 条字段事实核验任务重新对账，输出字段任务回链数、三字段分布、P/K/Q 状态桶、线索缺失数、冲突或多值线索数、PDF/湖北官方待核数和私有字段线索模板 SHA。当前 29764 条字段任务已有线索、11444 条缺线索、1137 条有冲突/多值/疑似信号，但 PDF 原页、湖北官方侧和必要高校辅证均未闭环，字段写回、志愿推荐和最终可用仍全部为 0。

新增页列底座人工复核 Overlay 公开账本：`data/working/issue19-page-side-foundation-human-review-overlay-public-ledger.csv` 在机器生成的私有字段线索模板之上建立可填写的人工复核层。私有 Overlay 覆盖 19 批、41208 个字段任务，并按 `字段事实核验任务ID` 回链 PDF SHA、批次、页码、版面列、`专业行ID`、字段名和字段线索模板行 SHA；公开账本只保存 462 个页列的记录数、已填计数、私有 CSV SHA 和非最终门禁。当前 Overlay 记录缺失 0，人工填写、字段确认、三方一致性可评估、字段写回复查、志愿推荐和最终可用计数均为 0。
...
新增第一闭环字段确认公开账本：`data/working/issue19-stable-foundation-first-closure-field-confirmation-public-ledger.csv` 把 206 条第一闭环任务接到私有字段确认工作台，形成“候选提示 -> PDF 原页人工读数 -> 湖北官方侧核验 -> 高校辅证核验 -> 三方一致性 -> 字段写回复查”的状态机。当前 26 条冲突任务需要双人核页，49 条可借机器坐标候选辅助核页，77 条按 PDF OCR 候选人工确认，54 条仍需直接看图；206 条 PDF 原页记录和 206 条湖北官方侧记录都未完成，字段写回、推荐依据和最终可用仍为 0。

截至 2026-06-28，当前最重要的新增执行入口是 `data/working/issue19-major-evidence-level-routing.csv`、`data/working/issue19-p0-immediate-pdf-reading-candidate-public-audit.csv`、`data/working/issue19-p0-immediate-page-review-packets.csv`、`data/working/issue19-p0-immediate-field-confirmation-public-ledger.csv`、`data/working/issue19-field-fact-p0-immediate-review-packet.csv`、`data/working/issue19-field-fact-p0-triage-execution-workbench.csv`、`data/working/issue19-field-fact-p0-semantic-crosssource-audit.csv`、`data/working/issue19-field-fact-p0-closure-action-workbench.csv`、`data/working/issue19-field-fact-p0-reread-machine-candidates.csv`、`data/working/issue19-field-fact-p0-reread-worklist.csv`、`data/working/issue19-field-fact-verification-tasks.csv`、`data/working/issue19-field-fact-closure-ledger.csv`、`data/working/issue19-page-side-foundation-all-batch-review-public-ledger.csv`、`data/working/issue19-foundation-stabilization-major-detail-tasks.csv` 和 `data/working/issue19-major-source-evidence-risk-sidecar.csv`。逐专业证据等级与核验路由表覆盖全部 13736 条招生专业明细，在官方结构化计划暂不可公开自动取得时，把每条明细分成 L3 高校辅证 854 条、L4 OCR 或单源线索 12882 条，并给出 A0-A5 自动高校官网核验可执行性、P0-P3 人工核验优先级和 H0-H4 人工强度；其中 P0 100% 人工核验 5043 条、P1 页列集中核验 7952 条、P2 自动官网核验后人工确认 557 条、P3 低风险抽检 184 条。字段事实闭环总账覆盖全部 13736 条招生专业明细，逐行标出再选科目、专业计划数、学费三项关键字段的 K0/K1/K2 状态：字段候选任务 19065 条、非空候选 7621 条，PDF 原页和湖北官方字段核验仍全部待核。字段事实核验任务队列进一步把这 13736 条明细拆成 41208 条逐字段任务：K0 无候选原页重读 11444 条、K1 有候选待核 7621 条、K2 OCR 候选待三方闭环 22143 条，并全部回连页级保真队列。P0 字段原页重读工作清单把 11444 条 K0 无候选任务单独排成执行清单，覆盖 8536 条招生专业明细、231 个 PDF 明细页和 967 所学校，当前先按专业计划数 5739 条、再选科目 4674 条、学费 1031 条回看原页。P0 字段机器坐标候选表进一步在这 11444 条任务上生成 4840 条非空坐标候选，其中专业计划数 2175 条、再选科目 1994 条、学费 671 条；6386 条仍无候选，218 条多值冲突。P0 字段闭环推进工作台把这些任务分成 A1/A1R 快速候选核页 4840 条、A2 多值冲突核页 218 条、A3 无候选重读 6386 条，并预留 PDF 人工读数、湖北官方字段值、高校官网或章程字段值和三方一致性状态；当前确认计数均为 0，只安排核验动作，不写回字段、不进入推荐或排序。P0 字段语义与多源线索审计表继续保持 11444 条字段任务，标出语义异常 15 条、计划数偏大需重点核页 11 条、高校字段线索 75 条，其中机器候选与高校辅证一致 22 条、冲突 1 条、官网有规范字段但机器无候选 52 条；它只用于重新安排核页优先级。P0 字段三方核验执行工作台把同一批字段任务排成稳定执行顺序：冲突异常立即核页 16 条、计划数偏大重点核页 11 条、高校辅证线索三方核验 74 条、多值坐标冲突核页 218 条、常规机器候选核页 4791 条、无候选原页重读 6334 条，其中 245 条要求双人复核；它只做派单和证据回链，所有字段写回和推荐门禁仍为 0。P0 字段即时复核包再从该工作台严格切出 `EXEC-01/02/03/04` 共 319 条最高优先级任务，覆盖 319 条招生专业明细、114 个 PDF 页、202 个院校代码和 222 个执行包，只用于优先核字段；P0 即时字段确认公开账本再把同一批任务转成私有人工记录状态机，当前 319 条 PDF 原页和湖北官方私有记录均待完成、75 条需高校辅证私有记录、290 条需双人复核，字段写回评估可进入数为 0；P0 即时按页核页包把这 319 条任务按 `PDF页码×版面列` 聚合成 148 个执行包，覆盖 114 页，公开层只保存证据编号、SHA、bbox、状态和门禁，私有 HTML/CSV 用于人工逐页逐列核 PDF 原件；P0 即时 PDF 原页读数候选公开审计把同一批 319 条任务上的私有候选降格为公开状态，当前 253 条有私有候选线索、66 条无稳定候选需人工看图、99 条需直接图像复核、290 条需双人复核，所有候选都不得自动写入人工读数。页列底座全 19 批公开复核账本把 462 个页列、231 个 PDF 明细页、13736 条专业明细和 41208 个字段任务全部接入批次复核流水线，但人工填写、字段确认、推荐依据和最终可用仍为 0。稳定化任务表把底座稳定性总看板中的 B0/B1/B2 共 12995 条招生专业明细全部拆成逐专业稳定化任务；源证据风险侧账覆盖全部 13736 条招生专业明细，把源证据覆盖结论、R2/R3/R4 风险、起始行 QC、窗口哈希、底座稳定性、闭环缺口和 P0 复核任务下沉到同一条 `专业行ID`。这些表全部保持 `最终可用=false`、`可进入下一阶段=false`、`是否允许作为志愿推荐依据=false`。同时新增两张原始数据保真审计表：`data/working/issue19-raw-major-lineage-consistency-audit.csv` 把 13736 条原始 OCR 专业行逐行回连到质量工作台、统一底座、总工作台、结构保真、稳定性看板、PDF 原页锚点、三年投档旁挂和闭环缺口看板，确认全链路核心字段漂移为 0；`data/working/issue19-raw-major-source-evidence-audit.csv` 进一步按 `来源页码+版面列+专业起始行号` 把 13736 条专业明细全部回连到私有 OCR 起始行、页级 manifest、私有窗口 JSONL 和公开锚点，起始行哈希、窗口哈希和页级 manifest 均满匹配。官方公开入口状态快照为 `data/working/issue19-official-public-entry-status.json`：湖北教育考试网 2026 计划页在 2026-06-28 公开 HTTP 复核可访问，但正文仍含“持续更新中/敬请期待”；湖北招生数智平台首页公开 HTTPS 可访问但无登录探针仍返回 401。
...
- 已生成 `data/working/issue19-candidate-v3-b0-b1-admission-detail-evidence-ledger.csv`：覆盖 324 条逐专业招生明细，已标准化留存官网/API/HTML/XLSX/PDF/图片/双表联合抽取证据 446 条；当前 158 条专业名匹配，66 条计划数与 OCR 一致，55 条为 OCR 缺失但官网可补，19 条计划数存在差异。山东工商学院 PDF 已以渲染图 OCR 专业名 + PDF 网格数字列双通道抽取 24 条湖北物理类明细，并保留 42 行审计表；浙江工业大学 PDF 已抽取湖北物理类 12 条明细、计划数合计 48。
- 已生成 B0/B1 保真复核队列：19 条计划数冲突、32 条专业未匹配、18 所学校补源缺口；其中 7 所是 P0 待补官方计划源、6 所是已有官网线索但尚未结构化到逐专业证据、5 所仅章程规则线索；“面向武汉、实验班、英才班、中外合作”等关键限定词未被官网证据覆盖时，不用泛化专业计划数替代；高校官网省份总计划未拆首选物理/历史时，不硬比物理类计划数；OCR 专业名疑似串入下一院校时直接降级为未匹配待核。
- 已生成候选 V3 全量逐专业字段保真总账：`data/working/issue19-candidate-v3-major-field-fidelity-ledger.csv` 覆盖 8412 条招生明细，其中 8234 条为高风险保真行、178 条暂未触发机器高风险；总账把 D0 原页风险、B0/B1 19 条计划数冲突、32 条官网未匹配、字段完整性、结构异常、费用底线、家庭底线和调剂风险全部落到逐专业明细行上，所有行仍保持 `最终可用=false`。
...
- 高校侧辅证刷新公开账本已经把 854 条官网辅证聚合成 80 条学校级动作队列，后续可以并行补官网来源、补结构化和做分层抽检；但它仍只是学校侧 double check 任务表，不能把高校官网一致项直接写成招生计划事实。
- 第一闭环批次包已经把最高优先级任务压缩到 37 个页列，后续可以按页列集中核 PDF 原页、湖北官方侧和高校官网辅证；但它仍只是执行包，不是字段事实表或志愿推荐表。
- 第一闭环复核材料公开账本已经把这 37 个页列接到私有 HTML/CSV 核页材料和公开状态机；当前全部仍为 `R0-Overlay已生成未填写`，字段写回、推荐依据、进入下一阶段和最终可用计数仍为 0。
- 第一闭环任务级复核公开账本已经把 206 条任务逐条接到 37 个页列材料、公共高校来源文件 SHA、PDF 原页待核、湖北官方侧待核、高校辅证待核和双人复核要求；它只用于执行和追踪，不确认字段值。
- 第一闭环私有预填已经把 74 条高校侧辅证候选线索放入 Git 忽略的私有工作台，公开层只同步 37 个页列的计数和 SHA；它只能减少人工查找成本，不能替代 PDF 原页、湖北官方侧或字段确认。
- 第一闭环核验执行队列已经把 37 个页列排成 E0/E1/E2 执行顺序，公开层只保存顺序、计数、证据编号、SHA、完成条件和非最终门禁；它能减少人工在多个表之间跳转，但当前 PDF 原页、湖北官方侧、高校辅证、字段写回和最终可用完成数仍全部为 0。
- 第一闭环 PDF OCR 候选公开审计已经覆盖 206 条任务，103 条任务已有 PDF OCR 候选提示，103 条任务仍需人工看图，26 条任务存在 PDF OCR 与高校辅证冲突。私有候选工作台只用于核页提示，公开层不保存候选值；所有自动写入、推荐和最终门禁仍为 0。
- 第一闭环页列候选看板已经把 206 条任务聚合回 37 个页列：10 个页列先核 PDF OCR 与高校辅证冲突，21 个页列先人工看图补 PDF 原页读数，6 个页列按 PDF OCR 候选逐条人工确认。第一闭环机器坐标候选审计进一步复用 P0 坐标候选，把 103 条原缺 PDF OCR 候选任务中的 49 条提升为机器坐标候选待人工核页，剩余 54 条仍需人工看图或人工判定字段；机器坐标候选仍不能自动写回、不能替代湖北官方计划。
- 第一闭环 G0 冲突字段补证执行包已经把 68 个准出门禁阻断字段压成 10 个页列补证包和 68 个逐字段执行项。下一步私有人工复核优先按 7 个 P0 双人复核缺口页列推进，再处理 1 个 P1 字段数较多冲突页列和 2 个 P2 常规冲突字段页列；它只安排 PDF 原页、湖北官方侧、高校辅证和双人复核补证顺序，不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段补证闭环结果已经把 68 个逐字段执行项接到本地未公开页列 CSV 的当前填写状态。当前 68 个字段全部仍阻断在 PDF 原页和湖北官方侧之前，47 个还需要双人复核，0 个可进入私有写回评审；后续推进应直接围绕这 10 个页列补齐 PDF 原页记录、湖北官方侧记录、高校辅证人工核验、冲突处理和双人复核，再由该账本判断是否解除阻断。
- 第一闭环 G0 冲突字段补证缺口任务队列已经把 68 个闭环结果拆成 523 条证据通道任务。后续推进时，优先并行补 PDF 原页、湖北官方侧和高校辅证三类一手/辅证记录，再做冲突处理、三方闭环、字段确认和写回评审；当前 251 条可并行执行、272 条等待前置闭环，仍不确认字段事实、不写回、不进入推荐。
- 第一闭环 G0 冲突字段补证执行波次已经把 523 条证据通道任务压成 47 个页列波次包。后续执行先按 W0/W1 推进 10 个页列的一手证据采集和 7 个页列的双人复核，再进入 W2 冲突处理与三方闭环、W3 字段确认、W4 写回评审；它让执行顺序更清楚，但仍不能替代 PDF 原页、湖北官方侧和人工复核结论。
- 第一闭环 G0 冲突字段 W0/W1 主动工作板已经把上述 W0/W1 当前可并行的 251 条任务压成 37 个 `页列×证据通道` 工作包和 10 个页列汇总。下一步实际执行应优先按该表分工：PDF 原页 68、湖北官方侧 68、高校辅证 68、双人复核 47；完成这些私有证据编号和 SHA 之前，不进入 W2 冲突处理、W3 字段确认或 W4 写回评审。
- W0/W1 主动工作板之后先看材料就绪审计：37 个工作包、10 个页列的私有材料已齐，人工记录仍未完成，高校辅证 seed 为 62，状态为 `ready_not_final`。因此下一步可以开核，但开核前后都不得把该审计解释为字段事实、写回依据、推荐依据或最终方案。
- W0/W1 开核执行清单已经把材料就绪的 37 个工作包转成实际分工入口。下一步执行要按该清单逐包补人工记录：先核 PDF 原页和双人复核，再并行补湖北官方侧和高校辅证；每完成一批人工记录后，只能回到字段闭环账本解除相应阻断，不能直接从开核清单写回字段事实或生成志愿推荐。
- W0/W1 候选分层已经把私有 Overlay 中的线索关系压成公开安全优先级。下一步人工开核时，先处理 30 个唯一双侧线索冲突字段及其 112 条任务通道记录，再处理 22 个仅高校辅证和 6 个仅 PDFOCR 的单侧线索字段；10 个双侧线索一致字段也必须补 PDF 原页和湖北官方侧，不能直接确认。
- 第一闭环证据仲裁总视图已经把 206 条第一闭环任务重新接到 G0 字段缺口和高校源回接状态。下一步执行时先看该视图判断一个页列属于 G0 冲突阻断还是非 G0 第一闭环任务，再进入 68 行 G0 字段闭环进度表；不要从 206 行总视图直接写回字段或生成候选志愿。
- 第一闭环 G0 字段闭环进度表是后续填私有核验记录的主入口。下一步优先补 68 个字段的 PDF 原页和湖北官方侧记录，同时回接已保留高校源中的 5 个结构化接入候选和 68 个 double check 线索；完成三方闭环和字段确认前，所有写回、推荐和最终方案门禁继续保持关闭。
- 第一闭环 G0 字段 W0/W1 可执行通道项已经把 523 条缺口任务中的当前可并行部分压成 251 条执行项。下一步实际操作应按这张表分派：先补 68 条 PDF 原页记录和 68 条湖北官方侧记录，同时核 68 条高校辅证线索；47 条双人复核完成后，再回到三方闭环、字段确认和写回评审。该表仍只是补证清单，不是字段事实结论。
...
新增的 `issue19-school-source-adapter-d0-d1-manual-review-packets-v1-public-ledger.csv` 把 344 条私有 diff 明细进一步压缩到 146 条私有人工核验项：27 条计划数冲突、102 条 OCR 计划数缺失但高校源可补、2 条疑似匹配、15 条计划数一致抽检。后续人工核验优先按 R0/R1/R2/R3 顺序执行，不把一致候选全量摊给人工。
...
如果湖北官方系统或省招办结构化计划在窗口期内仍不可直接自动取得，降级闭环按以下原则执行：第 19 期 PDF 原页仍作为省招办原件层；高校官网/API/XLSX/PDF/图片计划只用于自动 double check 和发现冲突；千问高考等第三方只用于专业分类、候选发现和异常提示。最终候选、冲稳保边界、B0/B1 优先组、计划数冲突、官网未匹配、补源缺口和字段空缺但进入候选的专业必须 100% 回看 PDF 原页并尽量核湖北官方系统。低风险辅证可以分层抽检，但一旦抽检出现结构错误、关键限定词丢失、计划数冲突、物理/历史未拆分、非 2026 湖北物理普通本科来源或同组家庭不可接受专业，就升级同页列、同校或同组 100% 人工核验。
...
当前第一闭环优先从 `data/working/issue19-first-closure-resolution-execution-overlay-v1.csv` 看准出缺口和补证波次，再看 `data/working/issue19-first-closure-fact-action-packets-v1-public-ledger.csv` 把 439 个事实范围压成的 79 个 `页列×事实核验动作组` 执行包，随后下钻到 `data/working/issue19-first-closure-fact-evidence-channel-workbench-v1-public-ledger.csv` 查看单个事实的证据通道和最小核验动作，然后继续看 `data/working/issue19-stable-foundation-first-closure-execution-queue.csv`、`data/working/issue19-stable-foundation-first-closure-page-side-candidate-dashboard.csv`、`data/working/issue19-stable-foundation-first-closure-pdf-ocr-candidate-public-audit.csv` 和 `data/working/issue19-stable-foundation-first-closure-machine-coordinate-candidate-public-audit.csv`：先处理 R0/W0B0 冲突事实页列，再处理 R1 专业名归属页列，再处理 R2 双人复核页列；页列内仍按 E0/E1/E2 执行顺序，优先处理 P0 PDF OCR 与高校辅证冲突任务，再处理缺 PDF OCR 但已有机器坐标候选的任务，再处理仍需人工看图的任务，最后处理 P2/P3。事实动作包和事实证据通道工作台内的 `同校高校源*` 都只是同校上下文，不可跨事实行求和；动作包里的高校源计数已按高校源进度看板 ID 去重。P0 即时字段仍从 `data/working/issue19-p0-immediate-page-execution-queue.csv` 安排人工核页顺序：先核 Q0 候选冲突页列，再核 Q1 无稳定候选页列，再核 Q2 候选一致但仍需官方闭环页列，最后核 Q3 常规候选页列。这些入口只负责把原始数据核准工作排得更稳，不代表目标学校或专业已经确定。
...
实际操作时，新增城市、学校或专业方向先从 `data/working/issue19-stable-foundation-major-screening-view.csv` 和 `data/working/issue19-stable-foundation-group-screening-view.csv` 查，先看逐专业机器线索、整组调剂风险、家庭底线、学校属性、字段缺口、证据路由和三年投档线索；再下钻到 `data/working/issue19-major-evidence-level-routing.csv`、`data/working/issue19-p0-immediate-pdf-reading-candidate-public-audit.csv`、`data/working/issue19-p0-immediate-page-review-packets.csv`、`data/working/issue19-p0-immediate-field-confirmation-public-ledger.csv`、`data/working/issue19-field-fact-p0-immediate-review-packet.csv`、`data/working/issue19-field-fact-p0-triage-execution-workbench.csv`、`data/working/issue19-field-fact-p0-semantic-crosssource-audit.csv`、`data/working/issue19-field-fact-p0-closure-action-workbench.csv`、`data/working/issue19-field-fact-p0-reread-machine-candidates.csv`、`data/working/issue19-field-fact-p0-reread-worklist.csv`、`data/working/issue19-field-fact-verification-tasks.csv`、`data/working/issue19-field-fact-closure-ledger.csv`、`data/working/issue19-major-source-evidence-risk-sidecar.csv`、`data/working/issue19-foundation-stabilization-major-detail-tasks.csv`、`data/working/issue19-candidate-filter-prep-major-detail.csv`、`data/working/issue19-moe-school-attribute-major-detail.csv` 和 `data/working/issue19-admission-detail-master-workbench.csv`。是否能进入候选讨论先看 `data/working/issue19-major-decision-readiness-gates.csv` 的阻断闸门。学校属性、公办民办、合作办学、职业本科和未匹配校名先看教育部学校属性表，但它只给登记信息线索，不能替代湖北官方计划和招生章程。结构风险看 `data/working/issue19-structural-risk-major-line-ledger.csv`、`data/working/issue19-major-line-layout-continuity-risk-ledger.csv`、`data/working/issue19-major-code-order-risk-ledger.csv` 和 `data/working/issue19-zero-detail-group-placeholder-workbench.csv`；官方系统回填消歧看 `data/working/issue19-hubei-official-query-key-collision-ledger.csv`。安排核验顺序时，以稳定基座筛选视图、逐专业证据等级与核验路由表、P0 即时 PDF 原页读数候选公开审计、P0 即时按页核页包、P0 即时字段确认公开账本、P0 字段即时复核包、P0 字段三方核验执行工作台、P0 字段语义与多源线索审计表、P0 字段闭环推进工作台、P0 字段机器坐标候选表、P0 字段原页重读清单、字段事实核验任务队列、字段事实闭环总账、源证据风险侧账、逐专业稳定化任务表和逐专业闭环缺口看板作为当前入口，再下钻到 P0/P1、官方平台、官网辅证和专业行原页证据锚点。
...
页列底座核验执行的公开状态入口是 `data/working/issue19-page-side-foundation-review-progress-public-ledger.csv`。它覆盖 19 批、462 个页列、13736 条专业明细和 41208 个字段任务，只同步 private 核页材料的填写状态和 SHA；当前 1441 个必填记录槽位全部未填，升级条件、推荐依据和最终可用计数均为 0。后续每填一批 private 核页记录，先更新该进度账本，再进入字段事实复查，不能跳过公开状态机直接生成候选推荐。

页列底座字段线索公开审计是当前“全量底座做准”的下一层硬门禁：`data/working/issue19-page-side-foundation-field-clue-public-audit.csv` 不承载任何学校/专业字段值，只把每个页列背后的字段任务覆盖、缺口和私有模板 SHA 摆出来。后续人工核页时，建议把 `private` 中机器生成的字段线索模板视为不可变模板，人工读数、湖北官方值、复核记录另放 overlay，避免重跑脚本覆盖人工判断。

页列底座人工复核 Overlay 是承接上述人工判断的实际记录层：`data/working/issue19-page-side-foundation-human-review-overlay-public-ledger.csv` 只公开 462 个页列的 Overlay 进度，私有目录才保存 41208 条字段级空白复核记录。当前它已经证明“每个字段任务都有可填写、可回链、可保留旧值的复核位置”，但还没有证明“字段事实已经正确”；彻底完成稳定数据基座还要继续把 PDF 原页、湖北官方系统或省招办计划、高校官网/章程三方证据写入 Overlay 并通过二次校验。

第 1 批样板复核已经扩展到全 19 批公开复核账本：`data/working/issue19-page-side-foundation-all-batch-review-public-ledger.csv` 覆盖 462 个页列、231 个 PDF 页、13736 条招生专业明细和 41208 个字段任务。公开表只保留计数、状态分布、私有详表 SHA 和非最终门禁；私有详表才保留学校/专业/字段候选和回页线索。全批次账本显示 Q0 无候选 15813 个字段、Q1 带候选待核 21606 个字段、Q2 OCR 齐全但待 PDF/湖北官方闭环 3789 个字段；正式 Overlay 写回、推荐依据和最终可用仍全部为 0。它解决的是“19 批都能并行复核并可回链”，不是“字段已经核准”。

如果湖北官方系统或省招办结构化计划在决策窗口内仍不可直接取得，底座推进改走降级闭环：第 19 期 PDF 原页是省招办原件层，高校官网/API/XLSX/PDF/图片计划用于自动 double check，千问高考等第三方只用于发现专业分类和异常提示。自动核验优先覆盖目标候选、冲稳保边界、字段缺口和高校官网已有线索；最终候选、冲稳保边界、B0/B1 优先组、计划数冲突、官网未匹配、字段空缺但进入候选的专业必须 100% 回看原页并尽量核湖北官方系统。低风险辅证按来源类型抽检，API/JSON 可低一些，PDF/图片和结构复杂来源要提高抽检比例；抽检出现结构错误、关键限定词丢失、计划数冲突、物理/历史未拆分、非 2026 湖北物理普通本科来源或同组家庭不可接受专业时，升级同页列、同校或同组 100% 人工核验。家庭只签认最终候选完整组、红色冲突项和抽检失败升级项，避免把 41208 个字段全部摊给家庭手工核对。
