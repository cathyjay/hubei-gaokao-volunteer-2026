...
第 1 批样板复核用于把速度和质量先拉到可验证状态：`data/working/issue19-page-side-foundation-batch-01-sample-public-audit.csv` 覆盖 25 个页列、23 个 PDF 页、717 条招生专业明细和 2151 个字段任务。样板结果显示 Q0 无候选 831 个字段、Q1 带候选待核 1071 个字段、Q2 OCR 齐全但待 PDF/湖北官方闭环 249 个字段；私有详表保留学校/专业/候选线索，公开表只保留计数、动作桶和 SHA。8 小时推进目标按这个样板扩展：争取完成 19 批私有 batch 结果、公开进度摘要、主线程统一汇总和校验；但 PDF 原页、湖北官方系统或省招办计划、学校辅证和最终字段确认仍必须有真实证据，不允许由 subagent 或脚本直接猜测写回。

如果湖北官方系统或省招办结构化计划在决策窗口内仍不可直接取得，底座推进改走降级闭环：第 19 期 PDF 原页是省招办原件层，高校官网/API/PDF/图片计划用于自动 double check，千问高考等第三方只用于发现专业分类和异常提示。自动核验优先覆盖目标候选、冲稳保边界、字段缺口和高校官网已有线索；人工只看最终候选、冲稳保边界、冲突字段和抽检失败批次，避免把 41208 个字段全部摊给家庭手工核对。
