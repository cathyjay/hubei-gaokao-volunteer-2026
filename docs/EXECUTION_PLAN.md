...
当前第一闭环优先从 `data/working/issue19-first-closure-resolution-execution-overlay-v1.csv` 看准出缺口和补证波次，再下钻到 `data/working/issue19-first-closure-fact-evidence-channel-workbench-v1-public-ledger.csv` 看 439 个事实范围的证据通道和最小核验动作，然后继续看 `data/working/issue19-stable-foundation-first-closure-execution-queue.csv`、`data/working/issue19-stable-foundation-first-closure-page-side-candidate-dashboard.csv`、`data/working/issue19-stable-foundation-first-closure-pdf-ocr-candidate-public-audit.csv` 和 `data/working/issue19-stable-foundation-first-closure-machine-coordinate-candidate-public-audit.csv`：先处理 R0/W0B0 冲突事实页列，再处理 R1 专业名归属页列，再处理 R2 双人复核页列；页列内仍按 E0/E1/E2 执行顺序，优先处理 P0 PDF OCR 与高校辅证冲突任务，再处理缺 PDF OCR 但已有机器坐标候选的任务，再处理仍需人工看图的任务，最后处理 P2/P3。事实证据通道工作台内的 `同校高校源*` 只是随事实行重复的同校上下文，不可跨行求和。P0 即时字段仍从 `data/working/issue19-p0-immediate-page-execution-queue.csv` 安排人工核页顺序：先核 Q0 候选冲突页列，再核 Q1 无稳定候选页列，再核 Q2 候选一致但仍需官方闭环页列，最后核 Q3 常规候选页列。这些入口只负责把原始数据核准工作排得更稳，不代表目标学校或专业已经确定。
