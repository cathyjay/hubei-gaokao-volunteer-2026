...
```text
2026 招生计划全量 OCR 明细底座已生成，候选V2逐专业明细复核种子、全量质量分层队列、逐专业明细质量工作台、底座机器审计表、候选V2证据总账和 240 页公开页级 manifest 已生成
```
...
- 已生成全量专业组质量分层和复核队列：3329 条专业组行、13736 条专业明细参与校验，3300 条进入复核队列，其中 3296 条为 P0；3 个规范化专业组代码重复、共 6 行，已显式标记，不做去重。
- 已生成全量逐专业明细质量工作台：13736 条专业行全部保留并生成唯一 `专业行ID`，5129 条结构异常全部匹配到专业行，13705 条进入逐专业复核队列，其中 13701 条为 P0。
...
| 时间 | 目标 | 产物 |
| --- | --- | --- |
| 2026-06-27 | 完成全量 OCR 明细底座、候选复核工作台、偏好专业和硬风险队列、候选V2逐专业明细种子、升级工作台、全量质量分层、逐专业质量工作台、底座机器审计、候选证据总账和页级 manifest；启动重点组人工复核 | 全量 OCR 招生明细、候选池复核工作台、优先复核队列、候选V2逐专业明细、升级闸门表、质量分层队列、逐专业明细质量工作台、底座审计表、候选字段复核总账、三方证据矩阵、页级 manifest |
