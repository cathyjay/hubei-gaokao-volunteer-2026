...
当前第一闭环优先从 `data/working/issue19-stable-foundation-first-closure-execution-queue.csv` 开始执行：先核 E0 冲突异常和需要双人复核的页列，再核 E1 计划数补缺或偏大页列，最后核 E2 官网未匹配专业名归属页列。P0 即时字段仍从 `data/working/issue19-p0-immediate-page-execution-queue.csv` 安排人工核页顺序：先核 Q0 候选冲突页列，再核 Q1 无稳定候选页列，再核 Q2 候选一致但仍需官方闭环页列，最后核 Q3 常规候选页列。这两个入口只负责把原始数据核准工作排得更稳，不代表目标学校或专业已经确定。
