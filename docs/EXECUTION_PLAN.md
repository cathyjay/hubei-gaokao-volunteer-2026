...
新增 C4/C6 结构化候选 diff：`data/working/issue19-c4-c6-structured-candidate-diff-public-ledger.csv` 已把北京语言大学、西安建筑科技大学新增 API 源接入综合结构化高校源，对 607 条 C4/C6 私有逐专业明细自动匹配。结果显示 229 条专业名匹配、95 条计划数一致候选、113 条 OCR 计划数缺失但官网可补、24 条计划数冲突；其中新增高校源命中 27 条、计划数一致 12 条、计划数冲突 6 条。下一步不是人工全量核 607 条，而是优先核冲突、OCR 缺失、疑似匹配和新增源命中抽检；若湖北官方侧继续不可自动取得，则继续按高校官网源自动 double check 加少量 PDF 原页人工核验闭环。

新增 C4/C6 高校官网补源尝试账本：`data/working/issue19-c4-c6-school-source-acquisition-attempts-public-ledger.csv` 把 D4/D5/D6 剩余补源缺口中的 13 所学校单独列成学校级自动探针和人工最小核验动作，覆盖 322 条相关私有明细。当前 4 所学校有入口但尚未结构化，1 所需要补 parser 或匹配规则，8 所仍需继续找学校官方计划源；它用于自动推进高校官网 double check 和缩小人工核验范围，仍不能替代第 19 期原页、湖北官方系统或省招办计划。
...
新增官方不可得抽样页列执行队列：`data/working/issue19-official-unavailable-sampling-review-execution-queue.csv` 把 46 个页列核验包排成可执行顺序：先核 E0 冲突/错位 6 个和 E1 官网未匹配 11 个，再核 E2 官网补缺 3 个、E3 强辅证抽检 2 个、E4 低风险抽检 24 个。该队列让后续人工不需要在私有材料里自行判断先后顺序，但仍只安排核验，不确认字段、不写回主表、不生成学校专业建议。

新增官方不可得抽样私有预填公开审计：`data/working/issue19-official-unavailable-sampling-triage-prefill-public-audit.csv` 把 46 个页列执行队列接到 Git 忽略的私有预填工作台。私有层预填 153 条抽样明细中的高校官网专业名、计划数、学费、选科和 OCR 线索；公开层只保留页列级计数、16 个高校来源文件的聚合计数、私有 CSV SHA 和非最终门禁。它解决的是“人工核页时少翻表、少找官网”，不解决“字段已经核准”；PDF 原页、湖北官方侧和必要高校辅证仍必须人工闭环。
...
新增第一闭环字段确认公开账本：`data/working/issue19-stable-foundation-first-closure-field-confirmation-public-ledger.csv` 把 205 条第一闭环任务接到私有字段确认工作台，形成“候选提示 -> PDF 原页人工读数 -> 湖北官方侧核验 -> 高校辅证核验 -> 三方一致性 -> 字段写回复查”的状态机。当前 25 条冲突任务需要双人核页，49 条可借机器坐标候选辅助核页，77 条按 PDF OCR 候选人工确认，54 条仍需直接看图；205 条 PDF 原页记录和 205 条湖北官方侧记录都未完成，字段写回、推荐依据和最终可用仍为 0。
...
- 第一闭环私有预填已经把 73 条高校侧辅证候选线索放入 Git 忽略的私有工作台，公开层只同步 36 个页列的计数和 SHA；它只能减少人工查找成本，不能替代 PDF 原页、湖北官方侧或字段确认。
- 第一闭环核验执行队列已经把 36 个页列排成 E0/E1/E2 执行顺序，公开层只保存顺序、计数、证据编号、SHA、完成条件和非最终门禁；它能减少人工在多个表之间跳转，但当前 PDF 原页、湖北官方侧、高校辅证、字段写回和最终可用完成数仍全部为 0。
- 第一闭环 PDF OCR 候选公开审计已经覆盖 205 条任务，102 条任务已有 PDF OCR 候选提示，103 条任务仍需人工看图，25 条任务存在 PDF OCR 与高校辅证冲突。私有候选工作台只用于核页提示，公开层不保存候选值；所有自动写入、推荐和最终门禁仍为 0。
- 第一闭环页列候选看板已经把 205 条任务聚合回 36 个页列：9 个页列先核 PDF OCR 与高校辅证冲突，21 个页列先人工看图补 PDF 原页读数，6 个页列按 PDF OCR 候选逐条人工确认。第一闭环机器坐标候选审计进一步复用 P0 坐标候选，把 103 条原缺 PDF OCR 候选任务中的 49 条提升为机器坐标候选待人工核页，剩余 54 条仍需人工看图或人工判定字段；机器坐标候选仍不能自动写回、不能替代湖北官方计划。
